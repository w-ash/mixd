...
- **W5 (XS, partly done 2026-07-27)** — `.github/dependabot.yml` now sets `cooldown: default-days: 7` on every ecosystem, so update *PRs* wait out fresh releases as repo policy. What remains is the direct-install path: an explicit `minimumReleaseAge` in `web/pnpm-workspace.yaml` so a manual `pnpm add` gets the same cooldown instead of relying on the pnpm default.
