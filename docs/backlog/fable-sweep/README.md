...
- **Playwright visual baselines are stale vs current Chromium** (found shipping v0.8.15): `pnpm --prefix web test:e2e` fails 12/18 visual snapshots at ~1% pixel drift with a freshly-installed Chrome 149 — the failing set is identical on pre-wave commits (incl. pages no wave touched, e.g. settings-sync), so it's browser-version antialiasing drift, not a code regression. Regenerate baselines (`--update-snapshots`) in a standalone chore or at the v0.8.17 closeout; check what browser CI pins before assuming CI is affected.
