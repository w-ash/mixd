...
## Shipped — current cycle (v0.9.x–v0.11.x)
...
- **v0.11.7** (2026-09-06) — The Sync page refuses a run the server would refuse and says why, a playlist import keeps every pick made under an earlier search, progress and chat streams are typed end to end, and the frontend's per-connector knowledge lives in one table: the web sweep that completes the v0.11.4–v0.11.6 series, with the API contract extended wherever the browser used to guess. [changelog](../../CHANGELOG.md#0117--2026-09-06)
- **v0.11.6.1** (2026-09-05) — Every workflow node's settings are declared once and the editor, the assistant, the validator, and the run all read the same declaration: play-history metrics and window are editable in the UI, combiners read their inputs from the graph, a deleted input no longer strands a hidden setting that blocks save, and a saved sort direction shows the way it runs. [changelog](../../CHANGELOG.md#01161--2026-09-05)
- **v0.11.6** (2026-09-03) — A workflow run no longer dies to a lock timeout while an import is running, and a tag filter on "drum & bass" matches nothing instead of aborting the run: writers serialise at the source, use cases ask connectors what they can do instead of probing, and transforms are pure domain code. [changelog](../../CHANGELOG.md#0116--2026-09-03)
...
- **v0.11.3** (2026-08-22) — Tidal joins as the sixth catalog: connect with your own free Tidal app (browser or terminal), see your favorites acknowledged, and dead track ids follow Tidal's official replacement pointers into your library. [changelog](../../CHANGELOG.md#0113--2026-08-22)
- **v0.11.2** (2026-08-22) — Your Spotify connection now says what it needs: an aged-out grant shows a one-click Reconnect instead of failing silently, quota exhaustion is named instead of retried forever, and disconnecting states exactly what it removes — credentials go, your imported music stays. [changelog](../../CHANGELOG.md#0112--2026-08-22)
- **v0.11.1** (2026-08-22) — Discogs connects with a single pasted token and your collection shows up immediately — release-grade metadata groundwork for the entity work ahead. [changelog](../../CHANGELOG.md#0111--2026-08-22)
- **v0.11.0** (2026-08-22) — Apple Music listening flows into your canonical history: connect once in the browser and every Apple play lands beside your Spotify and Last.fm listens, deduplicated, with honest timestamps. [changelog](../../CHANGELOG.md#0110--2026-08-22)
- **v0.10.4.1** (2026-08-21) — Sorting your library by anything other than the default no longer stalls: every sort column now has an index behind it, and paging deep into a sorted list stays as fast as the first page. Artist sorting is withdrawn until artists are first-class. Plus a guard that stops a local command or a local web session from writing into the production database. [changelog](../../CHANGELOG.md#01041--2026-08-21)
- **v0.10.4** (2026-08-21) — Your listening history is finally something you can look at: a Plays page that scrolls your whole history newest-first with a chart you click to jump to any stretch of it, the actual plays behind the numbers on every track page, and a Library that opens on what you've been playing rather than row one of the alphabet — sortable and filterable by play count and recency. [changelog](../../CHANGELOG.md#0104--2026-08-21)
- **v0.10.3.6** (2026-08-20) — The play-history cleanup is done and verified: your history re-derived to the row, 132,273 plays unchanged. Plus two fixes to the repair scripts themselves — one would have inserted ~80,000 skipped plays into your history as real listens, the other reattached 34 plays to the wrong account. [changelog](../../CHANGELOG.md#01036--2026-08-20)
- **v0.10.3.5** (2026-08-16) — A playlist import no longer fails because your play history was importing at the same moment: the previous release's unidentified error turned out to be two imports colliding on the same track, which is now retried once wholesale instead of ground through track-by-track. Plus workflow preview stops claiming it writes nothing, and gets the run guards to match what it really does. [changelog](../../CHANGELOG.md#01035--2026-08-16)
- **v0.10.3.4** (2026-08-16) — Deploys stop taking twelve minutes and ending in a forced kill: the app's own signal handler had been displacing the web server's, so nothing ever told the process to stop. Plus a failing playlist import step no longer poisons its own transaction and buries the error that explains it. [changelog](../../CHANGELOG.md#01034--2026-08-16)
- **v0.10.3.3** (2026-08-15) — A song your library already knows keeps its place in the playlist you just imported, and a listen the export wrote down twice is counted once rather than admitted as two. Three defects from reviewing the previous two releases, plus a repair script that now refuses a third tenant's rows instead of quietly taking them. [changelog](../../CHANGELOG.md#01033--2026-08-15)
- **v0.10.3.2** (2026-08-11) — Merging two tracks no longer deletes the observations underneath them: the raw listening records move with the merge, so re-deriving your history reproduces it exactly. Found while building the repair for the audit's own findings, and invisible until a rebuild came back smaller. Plus the tenancy repair tooling, one report covering all 51 pending merges, and timestamp correction for plays imported before the API fix. [changelog](../../CHANGELOG.md#01032--2026-08-11)
- **v0.10.3.1** (2026-08-10) — Closes the rest of the audit: a listen you paused your way through counts once rather than not at all, both doors that created duplicate tracks are shut behind one shared definition of "same recording", retired identity history can no longer be deleted out from under its own log, and an unopenable log file stops taking the site down. [changelog](../../CHANGELOG.md#01031--2026-08-10)
...
- **v0.10.2.6–2.10** (2026-08-08) — One deploy finishing the import-hardening train: imported tracks belong to the importing user (the tenancy bug behind both a 78-minute import and invisible history — fixed, repaired, and regression-pinned), hand mixd the whole GDPR export and walk away (server-side queue, per-file records), the full Last.fm import drops from a measured 4–6h to under one (batch-first path, 7-day windows), and unattended runs are safe (startup liveness reaper, pre-deploy busy gate). [changelog](../../CHANGELOG.md#01029--2026-08-08)
- **v0.10.2.5** (2026-08-08) — A long import survives a deploy, a restart, or the machine going away, and a killed run reports itself as failed instead of silently "complete" — the autostop incident that stranded 15,317 plays, closed across infra, cancellation, durability and the UI. Rides along: one way to fetch a track (dead-code gate green again) and a dependency-freshness sweep. [changelog](../../CHANGELOG.md#01025--2026-08-08)
- **v0.10.2.4** (2026-08-08) — Import throughput: batched Spotify lookups (~46k requests → ~920) and per-chunk bulk track creation (measured: ~50s of chunk DB time → under a second) put the remaining GDPR files at minutes each; plus the Last.fm checkpoint-integrity fix required before the full-history import, and projection progress/scoping fixes. [changelog](../../CHANGELOG.md#01024--2026-08-08)
- **v0.10.2.3** (2026-08-08) — Dead-ID rescue fixed (quoted search filters — the incident track is now the #1 result), partial-failure runs get an amber "Completed with issues" badge, and Import History lists exactly which tracks couldn't resolve. [changelog](../../CHANGELOG.md#01023--2026-08-08)
- **v0.10.2.2** (2026-08-02) — Import failures surface their reason and one bad track no longer kills the batch: resolver item loops get savepoint isolation (the `InFailedSqlTransaction` cascade that killed the first at-scale GDPR upload), the Import History row records the failure message, connector retries honor a 429's `Retry-After`, and import logs stop claiming success on soft failures. [changelog](../../CHANGELOG.md#01022--2026-08-02)
- **v0.10.2.1** (2026-07-31) — Post-ship review remediation + layer-contract closure: a re-match can no longer depose a pinned primary, the event log agrees with the rows it explains, the agent can list rejected pairs, and `lint-imports` guards the architecture in CI (2026-07-02 dependency audit closed). [changelog](../../CHANGELOG.md#01021--2026-07-31)
- **v0.10.2** (2026-07-31) — Mapping supersession & resolution event log: identity mappings are append-only, so a re-import supersedes rather than overwrites and every accept/reject/merge leaves an auditable event — plus a rejection you can take back (`mixd tracks unreject`, and the assistant). [changelog](../../CHANGELOG.md#0102--2026-07-31)
- **v0.10.1** (2026-07-27) — Continuous play polling: Spotify's recently-played API is polled on an adaptive cadence and refreshed on demand whenever plays are read, so today's listening counts today — plus workflow surfaces that stop lying about their own state. [changelog](../../CHANGELOG.md#0101--2026-07-27)
...
| **v0.10.0** | Convergent play history — order-free, re-import-safe canonical plays projected from the observation ledger; lands before the first at-scale import ([findings](play-import-convergence-findings.md)) | ✅ Completed | [details](v0.10.x.md#v0100-convergent-play-history) |
...
| **v0.11.0** | Apple Music foundation — connector auth-state fidelity, MusicKit JS bridge auth, client, ISRC-conservative resolution, play channel | 🚀 Shipped | [details](v0.11.x.md#v0110-apple-music-foundation) |
| **v0.11.1** | Discogs foundation — BYO-token client + collection snapshot | 🚀 Shipped | [details](v0.11.x.md#v0111-discogs-foundation) |
...
| **v0.13.4** | Tidal integration — favorites/likes sync, full playlist CRUD, workflow support (no play channel: Tidal has no listening-history endpoint) | 🔜 Not Started | [details](v0.13.x.md#v0134-tidal-integration) |
...
| v0.11.x | Connector foundations (Apple, Discogs, Tidal) + Spotify currency | Apple listening joins the canonical history; the Discogs and Tidal on-ramps; a Spotify connection that asks to reconnect instead of failing silently | Three new connector auth models to inspect; the shared substrate (token shapes, re-auth states) widened in the open | Groundwork for "connect Apple Music" and "connect Tidal" being first-class buttons |
| v0.12.x | First-class artists & albums | Favorites and artist-driven playlists on identity measured across six services | Entity design in the open (spike findings doc); rich relational model | Browse-by-artist/album navigation every music app is expected to have |
| v0.13.x | Completing the collection | Full reclamation — streaming (Apple and Tidal at parity), physical, and manual history unified; tags land in the listening moment, not from memory | Re-resolution machinery, collection import, and Tidal playlist CRUD to script; an in-tab Apple player and polled Spotify state | Apple Music and Tidal at parity with Spotify; a now-playing bar as the most legible surface in the app |
