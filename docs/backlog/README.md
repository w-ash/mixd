...
- **v0.10.2.6–2.9** (2026-08-08) — One deploy finishing the import-hardening train: imported tracks belong to the importing user (the tenancy bug behind both a 78-minute import and invisible history — fixed, repaired, and regression-pinned), hand mixd the whole GDPR export and walk away (server-side queue, per-file records), the full Last.fm import drops from a measured 4–6h to under one (batch-first path, 7-day windows), and unattended runs are safe (startup liveness reaper, pre-deploy busy gate). [changelog](../../CHANGELOG.md#01029--2026-08-08)
