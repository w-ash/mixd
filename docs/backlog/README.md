...
**Current Version**: 0.10.3.1
**Next**: the v0.10.3 production sequence, gated step by step — re-own the 20 cross-tenant tracks, then the `default` purge (needs `cleanup_mistenanted_spotify_tracks.py` to retire mappings first, now that migration 051 makes the FK `RESTRICT`), apply migrations 050/051, re-import to replay 30 write-failed resolutions, and `mixd plays rebuild --dry-run` before any rebuild. Then the residue the audit left: **RLS is configured on every tenant table and enforced on none** (prod connects with `BYPASSRLS`), the `start_shift_ms` backfill for 113 pre-fix API rows, the 29 duration-compatible canonical merges, and whether `uq_tracks_user_isrc` should constrain a field that is only a reference ([findings](v0.10.3-audit-findings.md)). Follow-up pool: the PLR0913/0917 flip decision ([spoke 26](fable-sweep/26-ratchet-closeout.md)), and the still-gated candidates (**MCP spec/SDK drift check — now unblocked**, the stable-v2 bump landed here; model/effort cost re-eval before Sonnet 5 intro pricing ends 2026-08-31; demand-gated conversation persistence, memory tool, subagent fan-out, chat-voices toggle).
