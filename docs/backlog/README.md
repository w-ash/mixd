...
- **v0.10.2.4** (2026-08-08) — Import throughput: batched Spotify lookups (~46k requests → ~920) and per-chunk bulk track creation (measured: ~50s of chunk DB time → under a second) put the remaining GDPR files at minutes each; plus the Last.fm checkpoint-integrity fix required before the full-history import, projection progress/scoping fixes, and a dependency-freshness sweep folded in before deploy. [changelog](../../CHANGELOG.md#01024--2026-08-08)
