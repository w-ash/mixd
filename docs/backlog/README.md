...
**Current Version**: 0.10.3.5
**Next**: the v0.10.3 production sequence, gated step by step — deploy (the Fly release command applies migrations 050 + 051), re-own the **2** genuinely re-ownable cross-tenant tracks, run the **51** approved merges (29 same-user duplicates + 22 cross-tenant, where the other 22 of the 24 collide with a track the user already owns and merging is the only remedy), purge `default` at the corrected 2026-07-27 window, backfill `start_shift_ms` on the 78 recoverable pre-fix API rows, re-import to replay 30 write-failed resolutions, and `mixd plays rebuild --dry-run` for review before any rebuild. Open decision: the 50 mistenanted `spotify_api` plays — purge (lossless; the export covers that week) or re-own, where 22 of the 50 cannot be timestamp-corrected and would become duplicate plays. Then the residue: **RLS is configured on every tenant table and enforced on none** (prod connects with `BYPASSRLS`), and whether `uq_tracks_user_isrc` should constrain a field that is only a reference ([findings](v0.10.3-audit-findings.md)). Follow-up pool: the PLR0913/0917 flip decision ([spoke 26](fable-sweep/26-ratchet-closeout.md)), and the still-gated candidates (**MCP spec/SDK drift check — now unblocked**, the stable-v2 bump landed here; model/effort cost re-eval before Sonnet 5 intro pricing ends 2026-08-31; demand-gated conversation persistence, memory tool, subagent fan-out, chat-voices toggle).
