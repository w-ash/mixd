...
**Next**: v0.10.2 Mapping supersession & resolution event log — append-only identity mappings so v0.11.x connectors are supersession-native from birth ([details](v0.10.x.md#v0102-mapping-supersession--resolution-event-log)). Follow-up pool unchanged: the [dependency-audit work orders](dependency-audit-findings.md) (W1–W10), the PLR0913/0917 flip decision ([spoke 26](fable-sweep/26-ratchet-closeout.md)), and the still-gated candidates (MCP spec/SDK drift check after the stable-v2 bump; model/effort cost re-eval before Sonnet 5 intro pricing ends 2026-08-31; demand-gated conversation persistence, memory tool, subagent fan-out, chat-voices toggle).
...
| **v0.10.1** | Continuous play polling — Spotify recently-played, demand-driven + adaptive; plus workflow feedback: saves visible immediately, run state visible wherever the user is watching, successful-run count in the list | 🚀 Shipped | [details](v0.10.x.md#v0101-continuous-play-polling) |
