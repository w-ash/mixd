...
**Next**: none scheduled — pick the next milestone from the follow-up pool. v0.9.4 Follow-Ups & Hardening shipped 2026-07-16 (a full-series review of v0.9.0–v0.9.3 with fixes applied — chat streaming/confirmation reliability, two prompt-injection wraps, several 500-path fixes, MCP spawn robustness — plus the ready-now follow-ups: page-contextual tool-routing refinement and the §6.5 user-flows rewrite). Remaining follow-up pool: the [dependency-audit work orders](dependency-audit-findings.md) (W1–W10), the PLR0913/0917 flip decision ([spoke 26](fable-sweep/26-ratchet-closeout.md)), and the still-gated v0.9.4 candidates (MCP spec-drift check after 2026-07-28; model/effort cost re-eval before Sonnet 5 intro pricing ends 2026-08-31; demand-gated conversation persistence, memory tool, subagent fan-out, chat-voices toggle).
...
| **v0.9.5** | Remote MCP server — the tool registry over authenticated Streamable-HTTP on the Fly/Neon production deployment (OAuth 2.1 resource server; per-user, from any agent) | 🔜 Not Started | [details](v0.9.x.md#v095-remote-mcp-server-your-production-library-from-any-agent) |
