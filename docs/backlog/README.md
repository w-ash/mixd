...
**Next**: v0.8.17 — Sweep closeout (ratchet remainder incl. the noqa-baseline whodunit, visual-regression gate revival, dependency audit, final Fable pass over unreviewed diffs) closes the v0.8.12–v0.8.17 arc. Remaining in the Fable window (through 2026-07-07): the [identity-resolution research pass](identity-resolution-research-handoff.md) (design-space memo feeding v1.0.0/v1.0.1/v1.0.3; scope-extended to artist identity for v0.10.0 + an omni-integration provider breadth scan for the unscheduled connectors). **The ordered queue is the [window execution order](v0.8.13-0.8.17.md) — the source of truth for what to tackle next.** Then v0.9.0 Workflow assistant — right-panel chat ported from couplefins
