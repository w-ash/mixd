...
- ~~**Spotify Feb-2026 dev-mode endpoint audit** (S, added 2026-07-16)~~ → Resolved by the 2026-08 provider research pass: the client is migrated to the renamed endpoints (`.claude/rules/spotify-api.md`); remaining currency work scheduled as [v0.11.2: Spotify API Currency](v0.11.x.md#v0112-spotify-api-currency); the batch-endpoint watch lives in [PDR-003](../decisions/PDR-003-spotify-dev-mode-batch-endpoints.md).
...
- ~~**Tidal Connector**~~ → Scheduled as [v0.11.3: Tidal Foundation](v0.11.x.md#v0113-tidal-foundation) + [v0.13.4: Tidal Integration](v0.13.x.md#v0134-tidal-integration) (2026-08-20). The old entry's `tidalapi` pointer was stale — that library drives the deprecated v1 API; the scheduled epics build on the documented v2 API.
...
- **Generalize adaptive play polling beyond Spotify** — `play_poll_policy.py` hardcodes `spotify:plays`/Last.fm-as-second-observer; `apple:plays` (v0.11.0) ships as a plain scheduled target without demand triggers or redundancy caps. Generalize the policy to any play channel so Apple (and future channels) get the same battery-friendly cadence. Also: factory-created play-resolver clients strand an httpx2 pool per poll (pre-existing, Spotify included) — close them in the orchestrator or share the pool.

- **Flaky: statement-probe count under parallel ordering** — `tests/integration/repositories/test_statement_probe.py::test_counts_transaction_begin_and_the_statement` intermittently counts 3 statements (extra `_bare`) instead of 2 when the integration tree runs under xdist with certain worker orderings; passes in isolation and in most full-suite runs. Present on v0.11.x HEAD (2026-08-22, predates the Discogs milestone; bisect against v0.10.x to find the leaking neighbor). Likely another test leaks a session/engine event hook into the probed session.

- **Unify the dead-grant reconnect state across connectors** — three shapes exist: Apple stamps `extra_data["reauth_required"]`; Spotify deletes the row and surfaces reauth for one probe cycle; Tidal deletes the row and renders as never-connected (loses the account name and Reconnect framing). Decide once: on a dead grant, keep the row, clear only the refresh token, stamp the marker; all probes derive `needs_reauth` from one read. (v0.11.x quality review, 2026-08-23.)
- **Extract the dead-grant ritual** — `spotify/auth.py` and `tidal/auth.py` duplicate detect-invalid-grant → compare-and-delete → clear-cache → typed raise plus the scope/extra_data/account_name carry-forward, down to identical log strings. `_shared/token_refresh.py` with `discard_dead_grant()` + `carry_forward()`; wants its own test pass. Natural rider on the unify-reconnect-state item above.
- **Collapse the six `_run_*` importer skeletons** — `import_play_history.py` repeats build-params → importer → orchestrator → two-phase → log/except six times; v0.13.x adds two more. One `_run_two_phase(command, uow, emitter, *, params, label)` (~210 → ~80 lines). Do it when v0.13.x touches the file.
- **ConnectorCard connect-flow registry** — the card branches on `auth_method` in four places, hardcodes the Apple hook + Discogs dialog (mounted for every connector), and `device_code` is connectable with no flow. A `connectFlows: Record<ConnectorAuthMethod, {useConnect, Dialog?}>` registry; `DiscogsTokenDialog` parameterizes into a generic token form as part of the move. Do with the next connector surface (v0.13.x).
- **Vendored Tidal OAS ships in every wheel/image** — 1.5 MB inside `src/`; only a pin test and the refresh script read it. Consider moving beside `scripts/` or excluding from the package build while keeping the drift gate.
