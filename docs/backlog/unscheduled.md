...
- **✅ Resolved (2026-09-05)** — ~~Expose `enricher.play_history` metrics + `period_days` as editor config fields~~ (S). `config_fields.py` now declares `metrics` as a `multi_select` (default `DEFAULT_PLAY_HISTORY_METRICS`) and `period_days` as a bounded number; the editor renders both via the new `MultiSelectInput`, the executor applies the declared default, and the enricher's dependency rules (`emits_metrics_from_config`, `metric_config_corequisites`) are registry metadata on the node (`nodes/catalog.py`) checked by `validate_registry` rather than literal tables in `validation.py`. The `string_list` prerequisite became the `multi_select` field type. _Original:_ _Problem:_ `enricher.play_history` has an empty config-field tuple (`config_fields.py`), so the editor renders no inputs for `metrics` or `period_days` — they live only in hand-authored/template JSON. Consequence (surfaced by v0.8.10's config-aware validation): an editor-built `filter.by_metric(metric_name="period_plays")` over a default enricher *always* warns and the user can't fix it in the UI (can't add `period_plays`). _Fix:_ add a multi-select `metrics` field (the four `ENRICHER_METRIC_DEFS` values) + a `period_days` number field to the enricher's config-field tuple, making the warning actionable in-editor. Needs the `"string_list"` field-type work tracked under *Deferred Clean Architecture Improvements*.
