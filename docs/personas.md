...
| Field | |
|---|---|
| **Context** | Tech-savvy music enthusiast. Uses Spotify and possibly Last.fm. Has a few hundred to a few thousand liked tracks and years of play history they've never been able to access or use. Comfortable with GitHub READMEs, config files, and exploring systems — enjoys understanding how things work. Signs up for the hosted instance, or clones the repo to run locally and tinker. |
...
| **Key behavior** | Signs up, connects services via OAuth. Explores the visual workflow editor, starts with templates, gradually builds custom workflows. Reads the docs. Files bug reports. Might contribute a workflow template or a feature request. Uses the CLI when it's faster. May clone the repo and run locally to understand the system or contribute. |
...
- **Easy signup + OAuth onboarding** — connect Spotify/Last.fm in one click
...
| **Not this person** | Not a developer or a tinkerer. Not someone who will learn workflow node types or read documentation. Not non-technical either — they're proficient web app users who can navigate a dashboard and connect accounts. They just aren't going to clone a repo or use a CLI. |

### What the Casual Enthusiast needs

- **LLM-assisted workflow creation** (v0.9.0) — The game-changer. Natural language intent → working workflow, no node types or config keys. "Make me a playlist of my most played tracks from the last 3 months that I haven't added to any playlist yet" just works.
- **Hosted deployment or app** — Signs up on the hosted instance. Does not deploy infrastructure.
...
This persona makes v0.9.0 (LLM-assisted creation) the most important adoption feature. Templates help the Tinkerer, but the Casual Enthusiast won't browse templates — they'll describe what they want and expect it to happen. The quality of the LLM workflow generation directly determines whether this person stays or bounces.
...
## The Data Exploiter — Anti-Persona
...
2. **No orphan features** — Every feature in the active backlog (v0.4.x through v1.1.x) traces to a defined persona.
...
4. **Dependency chain validated** — v0.5.0 (foundation) → v0.6.x (personal metadata: preferences + tags) → v0.7.x (connectors, independent of each other) → v0.8.0 (scheduling, hard dep on v0.5.0) → v0.8.1 (editor polish) → v0.9.0 (LLM, needs hosted instance + templates from v0.8.1) → v0.10.x (entity chain: artists → albums → Discogs → scrobbling) → v1.0.0 (auth, needs PostgreSQL from v0.5.0) → v1.1.x (social: privacy → follows → feed → sharing, needs auth from v1.0.0). Personal metadata before connectors means new connectors benefit from the preference/tag system from day one. Social after auth because multi-user data isolation is the foundation visibility controls build on.

### Changes applied

5. **v0.6.x inserted: Personal Metadata** (March 2026) — Preferences (dislike/like/love) + freeform namespaced tags (`mood:chill`, `energy:high`). Promotes the Curator's existing Spotify playlist-as-bucket workarounds into first-class concepts. Existing versions v0.6.x+ renumbered forward by one minor series; v1.0.0 milestone preserved.

6. **v0.8.0 split into v0.8.0 + v0.8.1** (was v0.7.0/v0.7.1) — Scheduling (v0.8.0) is the critical automation feature for the Curator. Don't block it behind editor polish. v0.8.1 adds sub-flows, import/export, templates, and playlist browse — features that benefit Tinkerer onboarding and inform v0.9.0 LLM prompt engineering.

7. **Workflow Templates added to v0.8.1** — 5–8 curated templates ("Current Obsessions", "Hidden Gems", "Forgotten Favorites", etc.). Highest-impact improvement for the Tinkerer. Templates also inform LLM prompt engineering in v0.9.0.

8. **Browse/Search Playlists moved from unscheduled to v0.8.1** — Replaces paste-ID-to-link with a browse UI. Natural fit alongside advanced workflow features — playlists are workflow inputs/outputs.

9. **v0.9.0 LLM Feedback Loop descoped** (was v0.8.0) — Removed A/B testing, feedback insights dashboard, model improvement framework (Data Exploiter thinking — premature at launch). Kept: simple thumbs-up/down feedback, quality tracking for manual prompt refinement.

10. **v1.0.0 renamed** — "Production-Ready Multi-User Platform" → "Multi-User Auth & Production Polish". Content was already sensibly scoped (no MFA, no RBAC). New title signals real engineering complexity (auth, isolation, security) without the scope-creep-inviting "platform" framing.
