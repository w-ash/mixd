...
async def verify_no_references(orphan_ids: list[int]) -> bool:
    ...
    async with get_session() as session:
        ...
        for orphan_id in sample_ids:
            ...
            if mapping_count > 0:
                logger.error(
                    f"❌ Found {mapping_count} active track_mappings for ID {orphan_id}!"
                )
                return False

        logger.info(
            f"✅ Verified sample of {len(sample_ids)} orphans - no active references found"
        )
        ...
        if playlist_count > 0:
            ...
            logger.warning(
                "Note: Orphan connector_tracks may be referenced in playlist JSON"
            )
            logger.warning(
                "This is safe - playlist JSON contains external IDs, not database IDs"
            )
...
async def hard_delete_orphans(
    orphan_ids: list[int], batch_size: int = 50, dry_run: bool = True
) -> int:
    ...
    if dry_run:
        logger.info(
            f"🔍 DRY RUN - Would delete {len(orphan_ids)} orphaned connector_tracks"
        )
    ...
    async with get_session() as session:
        ...
        for i in range(0, len(orphan_ids), batch_size):
            ...
            logger.info(
                f"   Batch {batch_num}/{total_batches}: Deleting {len(batch)} records..."
            )
...
async def main(dry_run: bool = True):
    ...
    logger.info(
        f"   Records {'would be deleted' if dry_run else 'deleted'}: {deleted_count}"
    )
    ...
    if dry_run:
        ...
        logger.info(
            f"   Database size reduced by removing {deleted_count} orphaned records"
        )
