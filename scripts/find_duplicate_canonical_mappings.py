...
def show_detailed_examples(violations, limit=5):
    ...
    print(f"🚨 DETAILED VIOLATION EXAMPLES (showing {min(limit, len(violations))} of {len(violations)})")
    print("=" * 80)
    
    for i, violation in enumerate(violations[:limit]):
        connector_name, connector_track_id, canonical_count, canonical_ids, created_times, title, latest = violation
...
def main():
    ...
    if not violations:
        print("✅ No violations found! All connector tracks map to exactly one canonical track.")
        return
    
    print(f"🚨 CRITICAL: Found {len(violations)} connector tracks with multiple canonical mappings!")
    print()
