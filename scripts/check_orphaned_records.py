...
async def check_orphaned_track_mappings() -> list[dict]:
    ...
    if orphans:
        ...
        for orphan in orphans[:5]:  # Show first 5
            logger.warning(f"   Mapping ID {orphan['id']}: {orphan['issue_type']} (track_id={orphan['track_id']}, connector_track_id={orphan['connector_track_id']})")
        if len(orphans) > 5:
...
async def check_orphaned_track_metrics() -> list[dict]:
    ...
    if orphans:
        ...
        for orphan in orphans[:5]:  # Show first 5
            logger.warning(f"   Metric ID {orphan['id']}: track_id={orphan['track_id']} ({orphan['connector_name']}.{orphan['metric_type']})")
        if len(orphans) > 5:
            logger.warning(f"   ... and {len(orphans) - 5} more")
    else:
...
async def check_orphaned_track_likes() -> list[dict]:
    ...
    if orphans:
        ...
        for orphan in orphans[:5]:  # Show first 5
            logger.warning(f"   Like ID {orphan['id']}: track_id={orphan['track_id']} ({orphan['service']})")
        if len(orphans) > 5:
            logger.warning(f"   ... and {len(orphans) - 5} more")
    else:
...
async def check_orphaned_track_plays() -> list[dict]:
    ...
    async with get_session() as session:
        ...
        if total_orphans > 0:
            ...
            for orphan in orphans[:5]:  # Show first 5
                logger.warning(f"   Play ID {orphan['id']}: track_id={orphan['track_id']} ({orphan['service']})")
            if len(orphans) > 5:
...
async def check_orphaned_playlist_tracks() -> list[dict]:
    ...
    async with get_session() as session:
        ...
        if total_orphans > 0:
            ...
            logger.warning(f"   Showing {len(orphans)} sample records:")
            for orphan in orphans[:5]:  # Show first 5
                logger.warning(f"   PlaylistTrack ID {orphan['id']}: {orphan['issue_type']} (playlist_id={orphan['playlist_id']}, track_id={orphan['track_id']})")
            if len(orphans) > 5:
                logger.warning(f"   ... and {len(orphans) - 5} more in sample")
...
async def check_orphaned_connector_tracks() -> list[dict]:
    ...
    async with get_session() as session:
        ...
        logger.warning(f"❌ Found orphaned connector_tracks (showing first {len(orphans)})")
        for orphan in orphans[:5]:  # Show first 5
            logger.warning(f"   ConnectorTrack ID {orphan['id']}: {orphan['connector_name']}:{orphan['connector_track_id']} ({orphan['title']})")
        if len(orphans) > 5:
            logger.warning(f"   ... and {len(orphans) - 5} more")
...
async def fix_orphaned_records(orphan_summary: dict[str, list[dict]], dry_run: bool = True) -> None:
    ...
    async with get_session() as session:
        ...
        if orphaned_mappings:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(mapping_ids)} orphaned track_mappings")
        ...
        if orphaned_metrics:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(metric_ids)} orphaned track_metrics")
        ...
        if orphaned_likes:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(like_ids)} orphaned track_likes")
        ...
        if orphaned_plays:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(play_ids)} orphaned track_plays")
            ...
            if not dry_run and len(play_ids) <= 10000:  # Safety limit
                ...
                logger.warning(f"   ⚠️  Too many track_plays to fix safely ({len(play_ids)}). Skipping automatic fix.")
        ...
        if orphaned_playlist_tracks:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(pt_ids)} orphaned playlist_tracks")
        ...
        if orphaned_connector_tracks:
            ...
            logger.info(f"   {'Would fix' if dry_run else 'Fixing'} {len(ct_ids)} orphaned connector_tracks")
...
async def main(fix_orphans: bool = False, dry_run: bool = True):
    ...
    orphan_summary['track_mappings'] = await run_with_timeout(check_orphaned_track_mappings(), 60)
    
    logger.info("   [2/6] Checking track_metrics...")
    orphan_summary['track_metrics'] = await run_with_timeout(check_orphaned_track_metrics(), 60)
    
    logger.info("   [3/6] Checking track_likes...")
    orphan_summary['track_likes'] = await run_with_timeout(check_orphaned_track_likes(), 60)
    
    logger.info("   [4/6] Checking track_plays...")
    orphan_summary['track_plays'] = await run_with_timeout(check_orphaned_track_plays(), 180)  # Longer timeout for large table
    
    logger.info("   [5/6] Checking playlist_tracks...")
    orphan_summary['playlist_tracks'] = await run_with_timeout(check_orphaned_playlist_tracks(), 120)
    
    logger.info("   [6/6] Checking connector_tracks...")
    orphan_summary['connector_tracks'] = await run_with_timeout(check_orphaned_connector_tracks(), 30)  # Much shorter timeout
...
def cli_main():
    ...
    @app.command()
    def check(
        fix_orphans: bool = typer.Option(False, "--fix-orphans", help="Fix orphaned records by soft-deleting them"),
        no_dry_run: bool = typer.Option(False, "--no-dry-run", help="Actually perform fixes (default is dry-run)"),
    ):
