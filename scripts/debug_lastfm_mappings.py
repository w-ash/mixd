...
async def debug_lastfm_mappings():
    ...
    async with get_session() as session:
        ...
        if not canonical_playlist:
            print(f"❌ No canonical playlist found for Spotify playlist {spotify_playlist_id}")
            return
        print(f"✅ Found canonical playlist: {canonical_playlist.id} - {canonical_playlist.name}")
        ...
        for track in canonical_playlist.tracks[:5]:  # First 5 tracks
            if track.id and track.id in track_ids:
                ...
                print(f"   Track {track.id}: {track.title} - {track.artists[0].name if track.artists else 'Unknown'} [{status}]")
                count += 1
        ...
        print(f"   Tracks with lastfm_global_playcount: {len(global_playcount_metrics)}")
