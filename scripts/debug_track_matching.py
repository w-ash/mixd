...
async def debug_track_matching(track_id: int, connector_name: str = "lastfm") -> None:
    ...
    async with get_session() as session:
        ...
        try:
            ...
            console.print(f"✅ Found track: [green]{track.title}[/green] by [green]{', '.join(a.name for a in track.artists)}[/green]")
            if track.album:
        ...
        try:
            ...
            logging.getLogger("src.infrastructure.matching_providers").setLevel(logging.DEBUG)
            ...
            if result.identity_mappings:
                ...
                for track_id, match_result in result.identity_mappings.items():
                    ...
                    console.print(f"     • Connector ID: [cyan]{match_result.connector_id}[/cyan]")
                    console.print(f"     • Confidence: [yellow]{match_result.confidence}[/yellow]")
                    console.print(f"     • Match Method: [blue]{match_result.match_method}[/blue]")
                    ...
                    if hasattr(match_result, 'evidence') and match_result.evidence:
                        ...
                        if hasattr(evidence, 'title_similarity'):
                            console.print(f"       - Title similarity: {evidence.title_similarity:.3f}")
                        if hasattr(evidence, 'artist_similarity'):
                            console.print(f"       - Artist similarity: {evidence.artist_similarity:.3f}")
                        if hasattr(evidence, 'duration_diff_ms'):
                            console.print(f"       - Duration diff: {evidence.duration_diff_ms}ms")
                    ...
                    if hasattr(match_result, 'service_data') and match_result.service_data:
                        ...
                        if 'title' in service_data:
                            console.print(f"       - Title: [green]{service_data['title']}[/green]")
                        if 'artist' in service_data:
                            console.print(f"       - Artist: [green]{service_data['artist']}[/green]")
