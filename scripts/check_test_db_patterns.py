    ...
    for test_file in test_dir.rglob("*.py"):
        ...
        for line in content.split('\n'):
            ...
            if re.search(r'async with get_session\(\)', line):
                print(f"❌ {test_file}:{line_number}: Direct get_session() usage in test")
                print(f"   Line: {line.strip()}")
                print("   Fix: Use 'db_session' fixture parameter instead")
                issues_found = True
            
            # Check for improper imports of get_session in tests
            if re.search(r'from.*get_session', line) and r'session_provider' not in line:
            ...
            if re.search(r'track_id\s*=\s*\d+', line) and not re.search(r'saved_track\.id', line):
