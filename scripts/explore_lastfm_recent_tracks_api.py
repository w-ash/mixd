...
async def explore_recent_tracks_extended():
    ...
    try:
                ...
                try:
                    if hasattr(track, method_name):
                        ...
                        if method_name == 'get_artist' and value:
                            ...
                            artist_name = value.get_name() if hasattr(value, 'get_name') else str(value)
                            print(f"      {display_name}: {artist_name}")
                        elif method_name == 'get_album' and value:
                            # Album is an object, get its name
                            album_name = value.get_name() if hasattr(value, 'get_name') else str(value)
        ...
        if 'extended' in params:
            ...
            try:
                ...
                for i, played_track in enumerate(extended_tracks[:2]):
                    ...
                    try:
                        loved = track.get_userloved() if hasattr(track, 'get_userloved') else None
            ...
            try:
                ...
                print(f"✅ Extended call via kwargs successful: {len(extended_tracks)} tracks")
                ...
                if len(extended_tracks) > 0 and len(basic_tracks) > 0:
                    ...
                    basic_methods = [m for m in dir(basic_track) if not m.startswith('_')]
                    ext_methods = [m for m in dir(ext_track) if not m.startswith('_')]
                    
                    if set(basic_methods) != set(ext_methods):
                        print(f"   Method differences: {set(ext_methods) - set(basic_methods)}")
                    else:
        ...
        print("• Track object has methods: get_title(), get_artist(), get_album(), etc.")
...
async def test_command_line_args():
    ...
    parser.add_argument("--limit", type=int, default=10, help="Number of tracks to fetch")
    ...
    print(f"🧪 Testing with: limit={args.limit}, page={args.page}, extended={args.extended}")
