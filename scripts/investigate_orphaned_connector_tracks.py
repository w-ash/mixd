...
async def check_connector_playlist_references(
    orphan_track_ids: list[str],
) -> dict[str, list[dict]]:
                ...
                if isinstance(item, dict):
                    ...
                    if item_track_id in orphan_track_ids:
                        ...
                        references[item_track_id].append({
                            ...
                            "connector_playlist_id": playlist_dict[
                                "connector_playlist_id"
                            ],
    ...
    logger.info(
        f"Found {len(references)} orphaned tracks referenced by connector playlists"
    )
    return references
...
async def analyze_metadata_recoverability(orphans: list[dict]) -> dict[str, Any]:
    ...
    for orphan in orphans:
        ...
        if created_date:
            ...
            analysis["creation_patterns"][date_str] = (
                analysis["creation_patterns"].get(date_str, 0) + 1
            )

        # Connector distribution
        connector = orphan.get("connector_name", "unknown")
        analysis["connector_distribution"][connector] = (
            analysis["connector_distribution"].get(connector, 0) + 1
        )
...
async def find_potential_canonical_matches(
    recoverable_orphans: list[dict],
) -> dict[str, list[dict]]:
...
async def main():
    ...
    logger.info("\n📋 METADATA ANALYSIS:")
    logger.info(
        f"   Has title: {analysis['has_title']} ({analysis['has_title'] / analysis['total_orphans'] * 100:.1f}%)"
    )
    logger.info(
        f"   Has artists: {analysis['has_artists']} ({analysis['has_artists'] / analysis['total_orphans'] * 100:.1f}%)"
    )
    logger.info(
        f"   Has ISRC: {analysis['has_isrc']} ({analysis['has_isrc'] / analysis['total_orphans'] * 100:.1f}%)"
    )
    logger.info(
        f"   Has duration: {analysis['has_duration']} ({analysis['has_duration'] / analysis['total_orphans'] * 100:.1f}%)"
    )
    logger.info(
        f"   Empty metadata: {analysis['empty_metadata']} ({analysis['empty_metadata'] / analysis['total_orphans'] * 100:.1f}%)"
    )
    ...
    sorted_dates = sorted(
        analysis["creation_patterns"].items(), key=operator.itemgetter(1), reverse=True
    )
    ...
    if playlist_references:
        ...
        for track_id, refs in playlist_references.items():
            ...
            for ref in refs:
                logger.info(
                    f"     - Playlist: {ref['playlist_name']} ({ref['connector_name']})"
                )
    ...
    if matches:
        ...
        for track_id, track_matches in list(matches.items())[:5]:  # Show first 5
            ...
            for match in track_matches:
                logger.info(
                    f"     → Match: {match['title']} by {match['artists']} (ID: {match['id']})"
                )

    logger.info("\n💡 RECOMMENDATIONS:")
    if playlist_references:
        logger.info(
            "   🚨 DO NOT DELETE - Some tracks are referenced by connector playlists"
        )
        logger.info(
            "   🔧 Priority: Create track_mappings for playlist-referenced tracks"
        )

    if analysis["recoverable_tracks"]:
        logger.info(
            f"   🔧 Attempt recovery for {len(analysis['recoverable_tracks'])} tracks with good metadata"
        )

    if analysis["empty_metadata"] > 0:
        logger.info(
            f"   🗑️  Consider safe deletion of {analysis['empty_metadata']} tracks with no metadata"
        )
