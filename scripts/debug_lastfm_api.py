...
async def debug_recent_tracks_api():
    ...
    if not connector.client:
        print("❌ Last.fm client not initialized - check LASTFM_KEY, LASTFM_SECRET, LASTFM_USERNAME")
        return
    ...
    try:
        ...
        if standard_tracks:
            ...
            print(f"   Service metadata keys: {list(sample_track.service_metadata.keys())}")
    ...
    try:
        ...
        if connector.client and connector.lastfm_username:
            lastfm_user = await asyncio.to_thread(connector.client.get_user, connector.lastfm_username)
            ...
            for i, (track, played_time) in enumerate(recent_tracks_basic[:2]):
                print(f"   Track {i + 1}: {track.get_artist().get_name()} - {track.get_title()}")
                
                # Try to access extended fields
                try:
                    loved = track.get_userloved() if hasattr(track, 'get_userloved') else "Not available"
                    print(f"     Loved: {loved}")
                except Exception as e:
                    print(f"     Loved: Error - {e}")
                
                try:
                    mbid = track.get_mbid() if hasattr(track, 'get_mbid') else "Not available"
                    print(f"     MBID: {mbid}")
                except Exception as e:
    ...
    try:
        ...
        print(f"   Artist MBIDs: {artist_mbid_count} ({artist_mbid_count / total * 100:.1f}%)")
        print(f"   Album MBIDs: {album_mbid_count} ({album_mbid_count / total * 100:.1f}%)")
...
async def test_lastfm_track_resolution():
    ...
    for i, play_record in enumerate(recent_tracks):
        ...
        try:
            ...
            print(f"      Domain track created: {domain_track.title} by {[a.name for a in domain_track.artists]}")
