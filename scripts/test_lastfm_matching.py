...
async def test_lastfm_matching():
    ...
    async with get_session() as session:
        ...
        canonical_playlist = await playlist_repo.get_playlist_by_connector(
            connector="spotify", 
            connector_id=spotify_playlist_id
        )
        
        if not canonical_playlist:
            print(f"❌ No canonical playlist found")
            return
        ...
        for track in test_tracks:
            ...
            try:
                ...
                if track.id in track_data and track_data[track.id]:
                    result = track_data[track.id]
                    print(f"   ✅ Found Last.fm data:")
                    ...
                    print(f"      User plays: {result.get('lastfm_user_playcount', 'N/A')}")
                    print(f"      Global plays: {result.get('lastfm_global_playcount', 'N/A')}")
                else:
        ...
        print(f"   - Testing shows whether Last.fm API can find these tracks")
        print(f"   - If found, we can create mappings to enable metrics collection")
