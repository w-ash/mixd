...
async def get_duplicate_mappings() -> dict[str, list[tuple[int, datetime]]]:
    ...
    logger.info(
        f"Found {len(duplicates)} connector tracks with duplicate canonical mappings"
    )
    return duplicates
...
async def merge_duplicates_for_connector_track(
    ...
    if dry_run:
        logger.info(
            f"DRY RUN: Would merge {len(losers)} duplicates for {connector_track_id}"
        )
    ...
    logger.info(
        f"Merging {len(losers)} duplicates for connector track {connector_track_id}"
    )
    logger.info(f"Winner: canonical_id={winner_id} (created {winner_created})")
    ...
    try:
        ...
        async with get_session() as session:
            ...
            await _delete_duplicate_mappings(
                session, winner_id, [loser[0] for loser in losers]
            )

            # Step 2: Use TrackMergeService for proper merge (handles metrics conflicts)
            for loser_id, loser_created in losers:
                logger.info(
                    f"Merging loser canonical_id={loser_id} → winner={winner_id}"
                )
...
async def _delete_duplicate_mappings(
    session, winner_id: int, loser_ids: list[int]
) -> None:
...
async def main(dry_run: bool = False, limit: int | None = None):
    ...
    if limit:
        logger.info(
            f"🎯 Processing first {limit} of {total_connectors} connector tracks"
        )
    ...
    for connector_track_id, mappings in connector_items:
        ...
        result = await merge_duplicates_for_connector_track(
            connector_track_id, mappings, merge_service, dry_run
        )
    ...
    if dry_run:
        ...
        logger.info(
            f"   ✅ Successfully merged {total_merges} duplicate canonical tracks"
        )
...
def cli_main():
    ...
    def merge(
        dry_run: bool = typer.Option(
            False, "--dry-run", help="Show what would be merged without making changes"
        ),
        limit: int = typer.Option(
            None, "--limit", help="Limit number of connector tracks to process"
        ),
    ):
