...
def rows_affected(result: Result[tuple[object, ...]]) -> int:
    """Return the DML row count from a session ``execute()`` result.

    ``AsyncSession.execute`` is typed to return ``Result``, whose stubs do not
    expose ``.rowcount`` — but a DELETE/UPDATE/INSERT produces a ``CursorResult``
    at runtime, which does. Casting to ``CursorResult[object]`` (not ``[Any]``)
    narrows it cleanly under ``reportAny``/``reportExplicitAny``. Single source of
    truth for every rowcount read across the repository layer.
    """
...
class BaseRepository[TDBModel: DatabaseModel, TDomainModel]:
    ...
    async def upsert(
        ...
        try:
            return await self._upsert_two_phase(lookup_attrs, insert_values, now)
        except Exception as e:
            logger.error(f"Upsert error: {e}")
            raise

    async def _upsert_two_phase(
        self,
        lookup_attrs: Mapping[str, object],
        insert_values: dict[str, object],
        now: datetime,
    ) -> TDomainModel:
        """Find-then-update-or-insert body for :meth:`upsert`.

        Uses a two-phase approach (lookup ID, then update or insert by ID) to
        avoid complex lazy loading chains that cause greenlet issues.
        """
        ...
        db_entity = await self.session.get(self.model_class, new_id, options=options)
    ...
    async def _bulk_upsert_in_savepoint(
        ...
        Wrap in savepoint so a failure only rolls back this INSERT, not the
        entire transaction (bulk_upsert may be called multiple times within a
        larger UoW operation).
        """
        async with self.session.begin_nested():
