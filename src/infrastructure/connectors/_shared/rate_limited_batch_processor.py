...
class RateLimitedBatchProcessor:
    ...
    async def process_batch(
        ...
        for item in items:
            ...
            self.logger.debug(
                ...
                milliseconds_since_batch_start=round((time.time() - self.batch_start_time) * 1000, 1),
            )
        ...
        try:
            ...
            async for item_id, result in self._collect_results():
                ...
                self.logger.info(
                    ...
                    progress_percent=round((completed_count / self.total_expected_items) * 100, 1),
                    milliseconds_since_batch_start=round((time.time() - self.batch_start_time) * 1000, 1),
                )
            ...
            self.logger.info(
                ...
                average_items_per_second=round(self.total_expected_items / batch_duration, 2),
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                self.logger.info(
                    ...
                    running_tasks=len(self.running_tasks),
                    queue_size=self.work_queue.qsize(),
                    milliseconds_since_batch_start=round((launch_time - self.batch_start_time) * 1000, 1),
                    actual_interval_ms=round((launch_time - (next_launch_time - self.rate_delay)) * 1000, 1) if launch_count > 1 else 0,
    ...
    async def _execute_work_item(self, work_item: WorkItem, process_func: Callable[[Any], Any]) -> None:
        ...
        self.logger.debug(
            ...
            milliseconds_since_queued=round((execution_start - work_item.queued_at) * 1000, 1),
        )
    ...
    async def _collect_results(self) -> AsyncIterator[tuple[str, Any]]:
        ...
        while len(yielded_items) < self.total_expected_items:
            await asyncio.sleep(_RESULT_POLLING_INTERVAL_MS / 1000.0)  # Convert ms to seconds
