...
class PlaylistOperationService:
    ...
    def optimize_operations(
        ...
        for op_type in [PlaylistOperationType.REMOVE, PlaylistOperationType.MOVE, PlaylistOperationType.ADD]:
    ...
    def _optimize_move_operations(
        ...
        sorted_moves = sorted(
            moves_with_position,
            key=lambda op: op.old_position or 0  # Guaranteed non-None, but satisfy type checker
        )
        ...
        for op in sorted_moves[1:]:
            ...
            if (op.old_position == expected_old_pos and 
                op.position == expected_new_pos and 
                len(current_block_ops) < 100):  # API limit
    ...
    def _optimize_add_operations(
        ...
        for op in sorted_adds[1:]:
            ...
            if (op.position == expected_position and 
                len(current_batch_ops) < 100):  # API limit
    ...
    def _optimize_remove_operations(
        ...
        for i in range(0, len(items_to_remove), 100):
            ...
            batches.append(OptimizedOperationBatch(
                batch_type="remove",
