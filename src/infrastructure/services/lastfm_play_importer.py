...
class LastfmPlayImporter(BasePlayImporter):
    ...
    def __init__(
        ...
        self.track_resolution_service = track_resolution_service or LastfmTrackResolutionService()
    ...
    async def import_plays(
        ...
        **kwargs,
        ...
        return await self.import_data(
            ...
            to_date=to_date,
            username=username,
            **{k: v for k, v in kwargs.items() if k not in {'from_date', 'to_date', 'username'}}
        )
    ...
    async def _resolve_checkpoint(
        ...
        try:
            ...
            logger.debug(f"Checkpoint resolution: found={checkpoint is not None}, user={resolved_username}")
            if checkpoint:
                logger.debug(f"Checkpoint details: last_timestamp={checkpoint.last_timestamp}, cursor={checkpoint.cursor}")
    ...
    async def _fetch_date_range_strategy(
        ...
        **additional_options,
        ...
        logger.info(f"📡 Fetching tracks with daily chunking: from_date={from_date}, to_date={to_date}, user={username}")
        logger.debug(f"Daily chunking debug: from_date type={type(from_date)}, to_date type={type(to_date)}")

        if not from_date or not to_date:
            raise ValueError("Both from_date and to_date are required for daily chunking strategy")
        ...
        if checkpoint and checkpoint.cursor:
            ...
            try:
                ...
                logger.info(f"📋 Re-processing checkpoint day: {checkpoint_date} (always redo to catch new plays)")
                
                # But don't go earlier than the requested from_date
                start_date = max(resume_date, original_start_date)
                logger.debug(f"Checkpoint resume: checkpoint_date={checkpoint_date}, resume_date={resume_date}, final_start={start_date}")
            except (ValueError, TypeError) as e:
                logger.warning(f"Invalid checkpoint cursor '{checkpoint.cursor}': {e}, starting from beginning")
                start_date = original_start_date
        else:
            start_date = original_start_date
            logger.debug(f"No checkpoint found, starting from requested date: {start_date}")
        
        end_date = original_end_date
        total_days = (end_date - start_date).days + 1
        
        logger.debug(f"Daily chunking: start_date={start_date}, end_date={end_date}, total_days={total_days}")
        logger.debug(f"Date calculation: original range={original_start_date} to {original_end_date}, effective range={start_date} to {end_date}")
        
        # If we're already caught up, return empty
        if start_date > end_date:
            checkpoint_cursor = checkpoint.cursor if checkpoint else "unknown"
            logger.info(f"📋 Already up to date: checkpoint date {checkpoint_cursor} is >= end_date {end_date}")
            return []
        ...
        while current_date <= end_date:
            ...
            if progress_callback:
                progress = int((days_processed / total_days) * 50) + 40  # 40-90% range for API fetching
                date_str = current_date.strftime("%Y-%m-%d")
                progress_callback(progress, 100, f"Fetching {date_str}... ({days_processed}/{total_days} days)")
            ...
            logger.debug(f"Day {current_date}: raw boundaries day_start={day_start}, day_end={day_end}")
            
            # Respect the original time boundaries if they're more restrictive
            effective_start = max(day_start, from_date) if current_date == start_date else day_start
            effective_end = min(day_end, to_date) if current_date == end_date else day_end
            
            logger.debug(f"Day {current_date}: effective boundaries start={effective_start}, end={effective_end}")
            logger.debug(f"Day {current_date}: is_first_day={current_date == start_date}, is_last_day={current_date == end_date}")
            ...
            logger.debug(f"Day {current_date}: fetched {len(day_records)} records, total so far: {len(all_play_records)}")
            ...
            if day_records:
                ...
                logger.debug(f"Day {current_date}: timestamp range {min_ts} to {max_ts}")
                
                # Check if timestamps are actually within the day boundaries
                if min_ts < effective_start or max_ts > effective_end:
                    logger.warning(f"Day {current_date}: timestamps outside expected range! "
        ...
        logger.info(f"📡 Daily chunking complete: {len(all_play_records)} records across {days_processed} days")
        
        # Log checkpoint status
        if checkpoint:
            logger.info(f"📋 Incremental import complete: processed {days_processed} new days since {checkpoint.cursor}")
        else:
            logger.info(f"📋 Full import complete: processed {days_processed} days total")
    ...
    async def _fetch_day_with_auto_scaling(
        ...
        if len(day_records) < 200:
            logger.debug(f"Day {current_date}: complete in single request ({len(day_records)} < 200)")
            return day_records
        
        # Power user case: need sub-chunking
        logger.info(f"Day {current_date} has 200+ plays, using sub-chunking...")
        logger.debug(f"Day {current_date}: got exactly 200 records, triggering sub-chunking")
    ...
    async def _sub_chunk_day(
        ...
        """
        logger.debug(f"Sub-chunking day {current_date}: start={day_start}, end={day_end}")
        ...
        logger.debug(f"Day {current_date}: starting sub-chunking with {chunk_hours}h chunks, working backwards from {current_time}")
        
        while current_time > day_start:
            chunk_count += 1
            chunk_start = max(day_start, current_time - timedelta(hours=chunk_hours))
            
            logger.debug(f"Day {current_date}: sub-chunk {chunk_count} ({chunk_hours}h): {chunk_start} to {current_time}")
            ...
            logger.debug(f"Day {current_date}: sub-chunk {chunk_count} returned {len(chunk_records)} records")
            ...
            if len(chunk_records) == 200 and chunk_hours > 1:
                ...
                logger.info(f"Day {current_date}: {chunk_hours}h chunk full, reducing to {new_chunk_hours}h chunks")
                logger.debug(f"Day {current_date}: chunk size reduction {chunk_hours}h -> {new_chunk_hours}h")
                chunk_hours = new_chunk_hours
            
            prev_time = current_time
            current_time = chunk_start
            logger.debug(f"Day {current_date}: sub-chunk iteration: moved from {prev_time} to {current_time}")
        
        logger.info(f"Day {current_date}: sub-chunked into {len(all_day_records)} total records using {chunk_count} chunks")
        logger.debug(f"Day {current_date}: sub-chunking complete, final chunk size was {chunk_hours}h")
        ...
        if all_day_records:
            ...
            if len(timestamps) != len(unique_timestamps):
                logger.warning(f"Day {current_date}: found {len(timestamps) - len(unique_timestamps)} duplicate timestamps in sub-chunked results")
    ...
    async def _process_data(
        ...
        **_kwargs,
        ...
        resolved_tracks, resolution_metrics = await self.track_resolution_service.resolve_plays_to_canonical_tracks(
            play_records=raw_data, uow=uow
        )
        
        if progress_callback:
            progress_callback(70, 100, f"Creating {len(resolved_tracks)} play records...")
        ...
        for i, play_record in enumerate(raw_data):
            ...
            track_play = TrackPlay(
                ...
                ms_played=play_record.ms_played or 0,  # Default to 0 if duration unknown
                context={
    ...
    async def _handle_checkpoints(self, raw_data: list[PlayRecord], uow: Any | None = None, **_kwargs) -> None:  # noqa: ARG002
