...
class LastfmTrackResolutionService:
    ...
    async def resolve_plays_to_canonical_tracks(
        ...
        unique_lastfm_identifiers = self._extract_unique_lastfm_identifiers(play_records)
        ...
        if progress_callback:
            progress_callback(30, 100, f"Resolving {len(unique_lastfm_identifiers)} unique tracks...")

        # Step 2: Resolve identifiers to canonical tracks using 3-step pattern
        canonical_tracks_map, resolution_metrics = await self._resolve_lastfm_to_canonical_tracks(
        ...
        for record in play_records:
            identifier = self._create_lastfm_identifier(record.artist_name, record.track_name)
        ...
        if progress_callback:
            progress_callback(100, 100, f"Resolution complete: {resolved_count}/{len(resolved_tracks)} tracks resolved")
    ...
    def _extract_unique_lastfm_identifiers(self, play_records: list[PlayRecord]) -> set[str]:
    ...
    async def _resolve_lastfm_to_canonical_tracks(
        ...
        logger.info(f"Resolving {len(lastfm_identifiers)} Last.fm identifiers to canonical tracks")
        ...
        for identifier, track in existing_canonical_tracks.items():
            ...
            logger.info(f"🔍 Found existing track: {artist_name} - {track_name} (ID: {track.id})")
        
        if existing_canonical_tracks:
            logger.info(f"Found {len(existing_canonical_tracks)} existing canonical tracks")

        # Phase 2: Create missing tracks from Last.fm metadata
        missing_identifiers = {
            identifier for identifier in lastfm_identifiers 
        ...
        if missing_identifiers:
            ...
            if progress_callback:
                progress_callback(60, 100, f"Creating {len(missing_identifiers)} new tracks...")
            ...
            for identifier in missing_identifiers:
                try:
                    ...
                    if canonical_track:
                        ...
                        logger.info(f"🆕 Created new track: {artist_name} - {track_name} (ID: {canonical_track.id})")
                        ...
                        if spotify_found:
                            ...
                            logger.info(f"🎵 Spotify discovery: {artist_name} - {track_name} → enhanced with Spotify mapping")
    ...
    async def _find_existing_tracks_by_metadata(
        ...
        logger.info(f"🔍 Bulk lookup: checking {len(connections)} Last.fm connector mappings")
        existing_by_connector = await uow.get_connector_repository().find_tracks_by_connectors(connections)
        ...
        for identifier, connector_id in identifier_to_connector_id.items():
            ...
            if connector_key in existing_by_connector:
                existing_canonical_tracks[identifier] = existing_by_connector[connector_key]
        
        if existing_canonical_tracks:
            logger.info(f"🔍 Bulk lookup complete: found {len(existing_canonical_tracks)} existing tracks")
    ...
    async def _create_canonical_track_from_lastfm(
        ...
        try:
            ...
            logger.error(f"Failed to create canonical track for {artist_name} - {track_name}: {e}")
            return None
