...
def merge_tracks(
    winner_id: int = typer.Option(..., "--winner-id", help="Track ID that will keep all references"),
    loser_id: int = typer.Option(..., "--loser-id", help="Track ID that will be merged and deleted"),
        ...
        async with get_session() as session:
            ...
            async with uow:
                ...
                if winner_track.has_same_identity_as(loser_track):
                    console.print("✅ [green]Tracks have matching identifiers (safe to merge)[/green]")
                else:
                    console.print("⚠️  [yellow]Warning: Tracks don't have matching identifiers[/yellow]")
                
                # Confirm merge unless forced
                if not force:
                    console.print("\n[yellow]This will:[/yellow]")
                    console.print("  • Move ALL plays, likes, playlist entries from loser to winner")
                ...
                result_track = await merge_service.merge_tracks(winner_id, loser_id, uow)
                
                console.print(f"✅ [green]Successfully merged tracks! Winner track ID: {result_track.id}[/green]")
    ...
    async def _show_track_async():
        async with get_session() as session:
            ...
            async with uow:
                ...
                table.add_row("Duration", f"{track.duration_ms}ms" if track.duration_ms else "—")
                table.add_row("ISRC", track.isrc or "—")
                
                # Show connector track IDs
                if track.connector_track_ids:
                    connector_ids = ", ".join(f"{k}: {v}" for k, v in track.connector_track_ids.items())
