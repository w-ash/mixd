...
Service that moves foreign key references and hard-deletes duplicate tracks.
...
class TrackMergeService:
    ...
    async def merge_tracks(
        ...
        await self._delete_track(loser_id, uow)
    ...
    async def _merge_track_metrics(
        ...
        conflict_query = text("""
            ...
            WHERE 
                ...
                winner.track_id = :winner_id
        """)

        result = await session.execute(
            conflict_query, {"loser_id": loser_id, "winner_id": winner_id}
        )
        ...
        for conflict in conflicts:
            ...
            if loser_collected_at > winner_collected_at:
                ...
                if isinstance(loser_collected_at, str):
                    ...
                    collected_at_value = dt.fromisoformat(loser_collected_at)
                ...
                logger.debug(
                    f"Updated winner metric {winner_metric_id} with loser's more recent value"
                )

            # Hard delete the loser's conflicting metric (whether we used its value or not)
            await session.execute(
                delete(DBTrackMetric).where(DBTrackMetric.id == loser_metric_id)
            )
            logger.debug(f"Deleted conflicting loser metric {loser_metric_id}")
        ...
        logger.debug(
            f"Merged track metrics: {loser_id} → {winner_id} ({len(conflicts)} conflicts resolved)"
        )
        ...
        for conflict in conflicts:
            ...
            if loser_connector_track_id == winner_connector_track_id:
                ...
                if should_update_winner:
                    ...
                    logger.debug(
                        f"Updated winner mapping {winner_mapping_id} with loser's better data"
                    )
                ...
                logger.debug(
                    f"Deleted duplicate loser mapping {loser_mapping_id} (same external ID)"
                )
                ...
                logger.debug(
                    f"Moved loser mapping {loser_mapping_id} to winner as secondary (different external ID)"
                )
    ...
    async def _merge_track_likes(
        ...
        for conflict in conflicts:
            ...
            if should_update_winner:
                ...
                update_values = {"is_liked": conflict.loser_is_liked, "updated_at": now}
                ...
                if conflict.loser_liked_at is not None:
                    ...
                    if isinstance(conflict.loser_liked_at, str):
                        ...
                        update_values["liked_at"] = dt.fromisoformat(
                ...
                if loser_last_synced is not None:
                    ...
                    if isinstance(loser_last_synced, str):
                        ...
                        update_values["last_synced"] = dt.fromisoformat(
                            loser_last_synced
                        )
                    else:
                ...
                logger.debug(
                    f"Updated winner like {winner_like_id} with loser's more recent state"
                )

            # Hard delete the loser's conflicting like (whether we used its state or not)
            await session.execute(
                delete(DBTrackLike).where(DBTrackLike.id == loser_like_id)
            )
            logger.debug(f"Deleted conflicting loser like {loser_like_id}")
    ...
    async def _delete_track(self, track_id: int, uow: UnitOfWorkProtocol) -> None:
        ...
        await session.execute(delete(DBTrack).where(DBTrack.id == track_id))
