    ...
    if not connector:
        ...
        if not connector_playlist or not connector_playlist.items:
            ...
            return {
                "tracklist": TrackList(),
                "playlist_id": None,
                "playlist_name": connector_playlist.name if connector_playlist else "Unknown",
                "source": connector,
                "source_id": playlist_id,
                "operation": "playlist_source",
                "track_count": 0,
        ...
        try:
            ...
            logger.info(f"Found existing canonical playlist {existing_playlist.id} for {connector}:{playlist_id}")
        except ValueError as e:
            logger.debug(f"No existing playlist found for {connector}:{playlist_id}: {e}")
            existing_playlist = None
        ...
        if existing_playlist:
            ...
            update_command = UpdateCanonicalPlaylistCommand(
                ...
                playlist_description=connector_playlist.description or f"Updated from {connector}",
            )
            ...
            create_command = CreateCanonicalPlaylistCommand(
                ...
                tracklist=tracklist,
                description=connector_playlist.description or f"Imported from {connector}",
