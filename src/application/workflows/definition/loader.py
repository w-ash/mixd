...
def get_definitions_dir() -> Path:
    """Get the path to the bundled workflow definitions directory.

    The ``definitions/`` data lives at the ``workflows`` package root (a
    package-level resource peer to ``engine/``/``nodes/``/``definition/``), so
    resolve up one level from this module.
    """
    return Path(__file__).parent.parent / "definitions"
