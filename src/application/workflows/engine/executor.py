...
@attrs.define
class _RunState:
    """Mutable per-run state shared by the node-lifecycle functions.
    ...
    dry_run: bool
    workflow_progress_broker: ProgressBroker | None
    workflow_operation_id: str | None
    task_results: dict[str, NodeResult] = attrs.field(factory=dict)
    node_records: list[NodeExecutionRecord] = attrs.field(factory=list)


async def _run_task_inner(
    state: _RunState, task_def: WorkflowTaskDef
) -> tuple[str, NodeResult | Exception]:
    """Run the node's observer/timeout/diagnostics lifecycle.

    Split out of ``_run_node_lifecycle`` so the protective ``try`` clause
    there stays small; these statements remain guarded by the caller's
    ``CancelledError``/``Exception`` totality net. Returns ``(task_id,
    NodeResult)`` on success/degrade or ``(task_id, Exception)`` on a fatal
    node failure.
    """
    task_id = task_def.id
    ...
    try:
        await _safe_emit(
            state.node_observer.on_node_starting(base_event),
        ...
        if isinstance(exc, TimeoutError):
            exc = TimeoutError(
                f"Node '{task_id}' ({node_type}) exceeded {timeout_seconds}s timeout"
            )
        ...
        await _safe_emit(
            state.node_observer.on_node_failed(failed_event, exc),
    ...
    if not was_degraded:
        ...
        await _safe_emit(
            state.node_observer.on_node_completed(completed_event, result),
            event="on_node_completed",
            node_id=task_id,
        )
        state.node_records.append(completed_event.to_record(status="completed"))
    ...
    if task_def.result_key:
        ...
        state.task_results[task_def.result_key] = result
...
async def _run_node_lifecycle(
    ...
    try:
        return await _run_task_inner(state, task_def)
    except asyncio.CancelledError:
    ...
    async def workflow_flow(
        ...
        **parameters: object,
        ...
        state = _RunState(
            ...
            dry_run=dry_run,
            workflow_progress_broker=workflow_progress_broker,
        ...
        try:
            ...
            for level in levels:
                ...
                if _shutdown_requested:
                    remaining_tasks = [
                        ...
                        for lvl in levels
                        for td in lvl
                        if td.id not in state.task_results
                    ]
                    logger.warning(
                ...
                async with asyncio.TaskGroup() as tg:
                    tasks = [
                        tg.create_task(_run_node_lifecycle(state, td)) for td in level
                    ]
                ...
                for task in tasks:
                    ...
                    if isinstance(outcome, Exception):
                        ...
                        state.task_results[task_id] = outcome
                        completed_count += 1
            ...
            return {
                "_task_results": state.task_results,
            ...
            # asyncio.shield docs.
            cleanup = asyncio.ensure_future(state.workflow_context.connectors.aclose())
            try:
...
async def _build_and_execute_workflow(
    workflow_def: WorkflowDef,
    ...
    *,
    ...
    Split out of ``run_workflow`` so the protective ``try`` clause there stays
