...
class ApplyPlaylistAssignmentsUseCase:
    async def execute(
        ...
        async with uow:
            ...
            # Empty path enters tracked_operation too — without it, an SSE
            # client polling for terminal events would never see one for a
            # zero-work run, leaving any waiting dialog stuck mid-flight.
            async with tracked_operation(
                emitter, "Applying playlist assignments"
            ) as operation_id:
                if not assignments:
                    return ApplyPlaylistAssignmentsResult(0, 0, 0, 0, 0, 0)
                return await self._apply_inner(
                    uow, command, assignments, emitter, operation_id
                )
    ...
    async def _apply_inner(
        ...
        uow: UnitOfWorkProtocol,
        command: ApplyPlaylistAssignmentsCommand,
        assignments: list[PlaylistAssignment],
        emitter: ProgressEmitter,
        operation_id: str,
    ) -> ApplyPlaylistAssignmentsResult:
        """Apply assignments with per-assignment progress events.

        Repos are re-fetched from the UoW (cheap dict lookups) so the
        body is self-contained and the parent ``execute`` doesn't have
        to forward five repo handles through the call signature.
        """
        assignment_repo = uow.get_playlist_assignment_repository()
        ...
        current_members_by_assignment: dict[UUID, list[PlaylistAssignmentMember]] = {}
        conflicts_logged = 0
            ...
            for assignment in cp_assignments:
                ...
                await emitter.emit_progress(
                    create_progress_event(
                        ...
                        current=processed,
                        total=total,
                        message=f"Applied {processed} of {total} assignments",
                    )
                )
        ...
        if desired_preferences:
            ...
            if prefs_to_write:
                await pref_repo.set_preferences(prefs_to_write, user_id=command.user_id)
...
async def run_apply_playlist_assignments(
    ...
    return await execute_use_case(
        lambda uow: ApplyPlaylistAssignmentsUseCase().execute(
