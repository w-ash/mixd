...
- **Writing as `default` on a remote database raises** (v0.10.4.1) — `set_rls_user_on_begin` refuses to open a transaction when the contextvar user is `DEFAULT_USER_ID` and `DATABASE_URL` is not localhost, because that combination is always an accident (prod carries three phantom tenants from it). Cross-tenant maintenance declares `system_context()`; `execute_use_case(user_id=None)` does so automatically. Note the guard sees the *declared* user — 23 tables still carry `DEFAULT 'default'` on `user_id`, so an INSERT that **omits** the column still lands in a phantom tenant.
