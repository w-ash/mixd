...
- All data access goes through `execute_use_case()` from `application/runner.py` — no direct repository access. **Exception (v0.6.5 credential/secret carve-out):** credential-management surfaces share infrastructure helpers directly, by design, so CLI + web reuse one implementation:
  ...
  - **Apple Music MusicKit JS bridge (v0.11.x)** — `routes/apple_auth.py` serves the browser bridge page and stores the resulting Music User Token: `TokenStorage` write/read plus `apple_music/{auth,client}` for the developer token and the best-effort storefront probe, with `persistence/database/user_context` scoping that probe to the state row's user. Same credential-surface rationale as OAuth: a browser-mediated token capture, not domain data.
  ...
  - **Discogs BYO personal access token (v0.11.1)** — `routes/connectors.py`'s `PUT /connectors/discogs/token` and `cli/discogs_commands.py` share one live validator, `infrastructure/connectors/discogs/token_service.py` (identity probe + cached collection count), plus `TokenStorage` write. Same shape as the assistant key: a write-only per-user secret validated live at connect time, not domain data.
