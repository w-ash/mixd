"""Unit tests for the async workflow heartbeat ticker.
...
import src.interface.api.routes.workflows as wf
...
class TestHeartbeatLoop:
    async def test_bumps_immediately_then_on_each_interval(self) -> None:
        ...
        with patch.object(wf, "_bump_heartbeat", AsyncMock(side_effect=record)):
            task = asyncio.create_task(
                wf._heartbeat_loop(run_id, interval_seconds=0.01)
            )
...
class TestBumpHeartbeat:
    async def test_suppresses_errors(self) -> None:
        ...
        with patch.object(wf, "_run_repo_session", side_effect=RuntimeError("db down")):
            await wf._bump_heartbeat(run_id)  # must not raise
