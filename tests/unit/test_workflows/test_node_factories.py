...
class TestNodeFactories:
    ...
    def test_make_node_invalid_type(self):
        ...
        with pytest.raises(ValueError, match="Unknown node type: invalid_type in category filter"):
...
class TestDestinationNodeFactory:
    ...
    def test_create_destination_node_invalid_type(self):
        ...
        with pytest.raises(ValueError, match="Unsupported destination type: invalid_destination"):
            create_destination_node("invalid_destination")

    @patch('src.application.workflows.node_factories.NodeContext')
    async def test_destination_node_execution(self, mock_node_context_class, sample_tracklist):
        ...
        with patch('src.application.workflows.node_factories.DESTINATION_HANDLERS', {"test_dest": mock_handler}):
...
class TestEnricherNodeFactory:
    ...
    @patch('src.application.workflows.node_factories.NodeContext')
    async def test_create_enricher_node_basic(self, mock_node_context_class, sample_tracklist):
        ...
        with patch('src.application.workflows.node_factories.NodeContext.format_enrichment_result') as mock_format:
    ...
    def test_create_enricher_node_missing_connector(self):
        ...
        with pytest.raises(ValueError, match="Enricher configuration must specify a 'connector' type"):
            create_enricher_node(config)

    @patch('src.application.workflows.node_factories.NodeContext')
    async def test_create_play_history_enricher_node(self, mock_node_context_class, sample_tracklist):
        ...
        # Mock format_enrichment_result
        with patch('src.application.workflows.node_factories.NodeContext.format_enrichment_result') as mock_format:
            mock_format.return_value = {"operation": "play_history_enrichment", "success": True}
...
class TestHelperFunctions:
    ...
    def test_get_connector_extractors_lastfm(self):
        ...
        with patch('src.infrastructure.connectors.lastfm.get_connector_config') as mock_get_config:
