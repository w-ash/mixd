...
class TestConcatenateDedup:
    def test_no_dedup_preserves_duplicates(self):
        ...
        tl1 = TrackList(tracks=[make_track(id=id1), make_track(id=id2)])
        tl2 = TrackList(tracks=[make_track(id=id2), make_track(id=id3)])
    ...
    def test_dedup_false_is_default(self):
        shared_id = uuid7()
        tl1 = TrackList(tracks=[make_track(id=shared_id)])
        tl2 = TrackList(tracks=[make_track(id=shared_id)])
        result = concatenate([tl1, tl2], tracklist=TrackList())
...
class TestInterleaveDedup:
    def test_no_dedup_preserves_duplicates(self):
        id1, id2, id3 = uuid7(), uuid7(), uuid7()
        tl1 = TrackList(tracks=[make_track(id=id1), make_track(id=id2)])
        tl2 = TrackList(tracks=[make_track(id=id1), make_track(id=id3)])
