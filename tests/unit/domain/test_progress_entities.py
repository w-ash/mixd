...
class TestProgressEvent:
    ...
    def test_completion_percentage_calculation(self):
        ...
        event = ProgressEvent(
            operation_id="test-op-123", current=25, total=100, message="Processing"
        )
        ...
        indeterminate_event = ProgressEvent(
            operation_id="test-op-123", current=50, total=None, message="Processing"
        )
    ...
    def test_negative_current_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Progress current \\(-5\\) must be non-negative"
        ):
            ProgressEvent(
                operation_id="test-op-123", current=-5, total=100, message="Processing"
            )

    def test_zero_total_validation(self):
        """Test validation rejects zero or negative total."""
        with pytest.raises(
            ValueError, match="Progress total \\(0\\) must be positive when specified"
        ):
            ProgressEvent(
                operation_id="test-op-123", current=0, total=0, message="Processing"
            )

        with pytest.raises(
            ValueError, match="Progress total \\(-10\\) must be positive when specified"
        ):
            ProgressEvent(
                operation_id="test-op-123", current=0, total=-10, message="Processing"
            )

    def test_current_exceeds_total_validation(self):
        """Test validation rejects current > total."""
        with pytest.raises(
            ValueError, match="Progress current \\(150\\) cannot exceed total \\(100\\)"
        ):
            ProgressEvent(
                operation_id="test-op-123", current=150, total=100, message="Processing"
            )
    ...
    def test_empty_operation_id_validation(self):
        ...
        with pytest.raises(ValueError, match="Progress operation_id cannot be empty"):
            ProgressEvent(operation_id="", current=50, total=100, message="Processing")
    ...
    def test_empty_message_validation(self):
        ...
        with pytest.raises(ValueError, match="Progress message cannot be empty"):
            ProgressEvent(operation_id="test-op-123", current=50, total=100, message="")
...
class TestProgressOperation:
    ...
    def test_duration_calculation(self):
        ...
        operation = ProgressOperation(
            description="Test operation", start_time=start_time, end_time=end_time
        )
        ...
        ongoing = ProgressOperation(
            description="Ongoing operation", start_time=start_time
        )
    ...
    def test_with_status_immutable_update(self):
        ...
        original = ProgressOperation(
            description="Test operation", status=OperationStatus.PENDING
        )
    ...
    def test_with_metadata_immutable_update(self):
        ...
        original = ProgressOperation(
            description="Test operation", metadata={"batch_size": 100}
        )
    ...
    def test_invalid_total_items_validation(self):
        ...
        with pytest.raises(
            ValueError,
            match="Operation total_items \\(0\\) must be positive when specified",
        ):
            ProgressOperation(description="Test operation", total_items=0)

        with pytest.raises(
            ValueError,
            match="Operation total_items \\(-10\\) must be positive when specified",
        ):
            ProgressOperation(description="Test operation", total_items=-10)
    ...
    def test_invalid_end_time_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Operation end_time cannot be before start_time"
        ):
            ProgressOperation(
                description="Test operation", start_time=start_time, end_time=end_time
            )
...
class TestProgressCoordinator:
    ...
    async def test_validate_nonexistent_operation(self, coordinator):
        ...
        event = create_progress_event(
            operation_id="nonexistent-op", current=25, total=100, message="Processing"
        )
