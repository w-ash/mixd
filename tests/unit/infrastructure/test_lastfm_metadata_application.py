...
class TestLastFMMetadataApplication:
    ...
    def test_lastfm_info_to_domain_track_conversion(self):
        ...
        print(
            f"   Enriched track has connector metadata: {bool(enriched_track.connector_metadata)}"
        )
        print(
            f"   LastFM metadata keys: {list(enriched_track.connector_metadata.get('lastfm', {}).keys())}"
        )
        ...
        print(
            f"   LastFM user playcount: {lastfm_metadata.get('lastfm_user_playcount')}"
        )
    ...
    def test_track_get_connector_attribute(self):
        ...
        user_playcount = track.get_connector_attribute(
            "lastfm", "lastfm_user_playcount"
        )
        global_playcount = track.get_connector_attribute(
            "lastfm", "lastfm_global_playcount"
        )
    ...
    async def test_fast_enrichment_end_to_end(self):
        ...
        with (
            patch.object(
                LastFMOperations,
                "get_track_info_intelligent",
                mock_get_track_info_intelligent,
            ),
            ...
            print(
                f"   Enriched track has metadata: {bool(enriched_track.connector_metadata)}"
            )
    ...
    def test_batch_metadata_format(self):
        ...
        print(
            f"   Sample track 1 playcount: {batch_results[1]['lastfm_user_playcount']}"
        )
        ...
        assert all(
            "lastfm_user_playcount" in metadata for metadata in batch_results.values()
        )
