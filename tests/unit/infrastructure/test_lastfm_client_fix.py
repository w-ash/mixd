...
class TestLastFMClientFix:
    ...
    def test_client_attributes_initialized(self):
        ...
        with patch("src.infrastructure.connectors.lastfm.client.settings") as mock_settings:
            ...
            mock_settings.credentials.lastfm_password.get_secret_value.return_value = "test_password"
            mock_settings.api.lastfm_rate_limit = 5
            
            with patch("pylast.LastFMNetwork"):
                with patch("pylast.md5", return_value="test_hash"):
                    client = LastFMAPIClient()
                    
                    # Verify all attributes exist
                    assert hasattr(client, 'lastfm_password_hash')
                    assert client.lastfm_password_hash == "test_hash"
        ...
        with patch("src.infrastructure.connectors.lastfm.client.settings") as mock_settings:
            ...
            with patch("pylast.LastFMNetwork"):
                ...
                try:
                    ...
                    method = getattr(client, 'get_track_info_comprehensive')
                    assert method is not None
                    print("✅ get_track_info_comprehensive method accessible without errors")
