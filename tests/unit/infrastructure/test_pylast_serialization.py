...
class TestPylastSerialization:
        ...
        def create_and_use_pylast_client(call_id: int) -> dict[str, Any]:
            ...
            try:
                client = pylast.LastFMNetwork(
                    api_key="test_key", api_secret=None, username=None
                )
                client_id = id(client)

                print(
                    f"   Pylast Call {call_id}: Client {client_id} created on thread {thread_id}"
                )
                ...
                with patch.object(client, "_request") as mock_request:
                    ...
                    # Make the call
                    print(
                        f"   Pylast Call {call_id}: Starting get_track on client {client_id}"
                    )
                    ...
                    print(
                        f"   Pylast Call {call_id}: Completed on thread {thread_id} after {duration:.3f}s"
                    )
    ...
    async def test_pylast_shared_resources(self):
        ...
        def pylast_with_resource_inspection(call_id: int) -> dict[str, Any]:
            ...
            for attr in dir(pylast):
                ...
                try:
                    ...
                    if callable(value):
                        continue
                    module_attrs[attr] = {"type": type(value).__name__, "id": id(value)}
        ...
        for attr_name, resource_ids in shared_resources.items():
            ...
            if len(unique_ids) == 1:
                print(
                    f"   {attr_name}: SHARED across all clients (ID: {next(iter(unique_ids))})"
                )
            else:
