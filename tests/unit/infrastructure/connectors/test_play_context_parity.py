...
Only Spotify still has a second implementation: its context carries a per-run
``resolution_method``. Apple and Last.fm build theirs through the shared
resolution loop, which calls the domain builder directly, so there is nothing
left to drift.
