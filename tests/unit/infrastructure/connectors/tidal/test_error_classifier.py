...
class TestTemplateFallthrough:
    def test_429_falls_through_to_rate_limit(self, classifier: TidalErrorClassifier):
        error_type, error_code, _ = classifier.classify_error(
            make_status_error(429, headers={"Retry-After": "12"})
        )
    ...
    def test_429_without_retry_after_still_rate_limit(
        ...
        # Template default: no invented backoff number — Retry-After absence
        # is the shared retry policy's problem, not the classifier's.
        error_type, error_code, _ = classifier.classify_error(make_status_error(429))

        assert error_type == "rate_limit"
        assert error_code == "429"

    def test_5xx_is_temporary(self, classifier: TidalErrorClassifier):
        error_type, error_code, _ = classifier.classify_error(make_status_error(503))

        assert error_type == "temporary"
        assert error_code == "503"

    def test_plain_request_error_is_temporary(self, classifier: TidalErrorClassifier):
        request = httpx2.Request("GET", "https://openapi.tidal.com/v2/tracks")
    ...
    def test_404_is_not_found(self, classifier: TidalErrorClassifier):
        error_type, _, _ = classifier.classify_error(make_status_error(404))

        assert error_type == "not_found"

    def test_403_is_permanent(self, classifier: TidalErrorClassifier):
        error_type, error_code, _ = classifier.classify_error(make_status_error(403))
