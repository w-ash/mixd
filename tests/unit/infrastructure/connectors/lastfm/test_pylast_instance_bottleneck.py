...
class TestPylastInstanceBottleneck:
    ...
    async def test_single_pylast_instance_serialization(
        ...
        async def timed_api_call(call_id: int) -> dict[str, Any]:
            ...
            print(
                f"   Call {call_id}: Completed in {duration:.2f}s on thread {thread_id}"
            )
    ...
    async def test_multiple_pylast_instances_hypothesis(
        ...
        async def timed_api_call_with_new_client(call_id: int) -> dict[str, Any]:
            ...
            print(
                f"   Call {call_id}: Completed in {duration:.2f}s (client {id(client)})"
            )
            call_results.append(call_data)
            return call_data
        ...
        tasks = [
            asyncio.create_task(timed_api_call_with_new_client(i))
    ...
    async def test_instance_comparison(self, mock_pylast_slow_response):
        ...
        single_result = await self.test_single_pylast_instance_serialization(
            mock_pylast_slow_response
        )

        # Test multiple instances
        multiple_result = await self.test_multiple_pylast_instances_hypothesis(
            mock_pylast_slow_response
        )
        ...
        speedup = (
            single_result["overall_duration"] / multiple_result["overall_duration"]
        )
        print(f"   Speedup: {speedup:.1f}x")

        if speedup > 2.0:
            print(
                "   🎯 HYPOTHESIS CONFIRMED: Multiple instances solve the bottleneck!"
            )
            assert multiple_result["concurrent"], (
                "Multiple instances should enable concurrency"
            )
        else:
