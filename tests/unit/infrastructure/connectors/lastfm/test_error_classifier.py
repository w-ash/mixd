...
class TestLastFMErrorClassifier:
    ...
    def test_permanent_error_codes(self, classifier, error_code):
        ...
        assert description.startswith(("Invalid", "Authentication", "Unauthorized", "Subscribers", "Login", "Trial", "Not Enough", "No Peak", "Radio", "API Key", "Deprecated", "This"))
    ...
    def test_temporary_error_codes(self, classifier, error_code):
        ...
        assert returned_code == error_code
        assert description.startswith(("Operation failed", "Invalid session", "Service", "The service", "Not Enough Content"))
    ...
    @pytest.mark.parametrize('text_pattern', [
        "rate limit exceeded", "too many requests", "quota exceeded", "throttle active"
    ])
    ...
    def test_error_code_6_with_rate_limit_text_is_permanent(self, classifier):
        ...
        exception = pylast.WSError("LastFm", "6", "Invalid parameters - rate limit exceeded")
    ...
    @pytest.mark.parametrize('text_pattern', [
        "track not found", "does not exist", "no such track"
    ])
    ...
    @pytest.mark.parametrize('text_pattern', [
        "timeout", "connection failed", "network error", "server error", "503", "502", "500"
    ])
    ...
    @pytest.mark.parametrize('text_pattern', [
        "unauthorized", "forbidden", "invalid key", "invalid api key", "authentication failed"
    ])
