...
class TestThreadLocalFix:
    ...
    def mock_pylast_with_thread_tracking(self):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            def create_mock_instance(*args, **kwargs):
                ...
                mock_instance.get_track = lambda *a, **kw: tracked_get_track(mock_instance, *a, **kw)
    ...
    async def test_thread_local_clients_created(self, mock_pylast_with_thread_tracking):
        ...
        assert unique_instances > 1, f"Should create multiple pylast instances: {unique_instances}"
    ...
    @pytest.mark.asyncio
    async def test_thread_local_performance_improvement(self, mock_pylast_with_thread_tracking):
        ...
        thread_local_result = await self.test_thread_local_clients_created(mock_pylast_with_thread_tracking)
        
        print("\n🏁 Performance Summary:")
        print(f"   Thread-local duration: {thread_local_result['overall_duration']:.2f}s")
        ...
        speedup = theoretical_single_instance_time / thread_local_result['overall_duration']
