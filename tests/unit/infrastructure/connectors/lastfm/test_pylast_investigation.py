...
class TestPylastBlockingBehavior:
    ...
    async def test_asyncio_to_thread_behavior_under_load(self):
        ...
        assert total_time < sequential_time * 0.8, f"Not truly concurrent: {total_time:.3f}s vs {sequential_time:.3f}s"
        
        # Total time should be close to longest individual call
        assert total_time < max_individual_duration + 0.3, f"Unexpected total time: {total_time:.3f}s"
    ...
    async def test_timeout_wrapper_effectiveness(self):
        ...
        try:
            ...
            assert 0.9 < timeout_duration < 1.2, f"Timeout timing off: {timeout_duration:.3f}s"
        
        # Test successful completion within timeout
        start_time = time.time()
        result = await asyncio.wait_for(
            asyncio.to_thread(medium_call),
            timeout=2.0
        )
        success_duration = time.time() - start_time
        
        assert result == "Medium result"
        assert 0.4 < success_duration < 0.7, f"Medium call timing: {success_duration:.3f}s"
    ...
    async def test_http_session_behavior_simulation(self):
        ...
        async def simulate_pylast_pattern():
            ...
            start_time = time.time()
            tasks = [asyncio.create_task(asyncio.to_thread(sync_http_request, url)) for url in urls]
            results = await asyncio.gather(*tasks)
            total_time = time.time() - start_time
        ...
        print(f"   Avg per request: {simulation_result['total_time'] / simulation_result['num_requests']:.3f}s")
    ...
    async def test_thread_pool_exhaustion_detection(self):
        ...
        cpu_tasks = [asyncio.create_task(asyncio.to_thread(cpu_bound_task, i)) for i in range(10)]
        io_tasks = [asyncio.create_task(asyncio.to_thread(io_bound_task, i)) for i in range(10, 20)]
        ...
        assert total_time < expected_time * 2, f"Possible thread pool exhaustion: {total_time:.3f}s"
