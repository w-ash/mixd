...
class TestThreadPoolConcurrency:
    ...
    async def test_asyncio_to_thread_concurrency_limit(self):
        ...
        for completed_task in asyncio.as_completed(tasks):
            ...
            print(f"   ✓ Call {result['call_id']} completed at {elapsed:.3f}s (thread {result['thread_id']})")
            ...
            for j, result2 in enumerate(results):
                if i != j:
                    ...
                    if (result1["start_time"] < result2["end_time"] and 
                        result1["end_time"] > result2["start_time"]):
                        overlapping_calls += 1
        ...
        print(f"   Efficiency ratio: {(num_calls * call_duration) / overall_duration:.2f}")
        ...
        if overall_duration > call_duration * 2:
            ...
            print(f"   This suggests thread pool exhaustion with only {unique_threads} threads")
        else:
    ...
    async def test_thread_pool_capacity_detection(self):
        ...
        if len(unique_threads) < 20:
            print(f"   ⚠️ Limited thread pool detected: only {len(unique_threads)} threads")
        else:
    ...
    async def test_lastfm_pattern_simulation(self):
        ...
        for completed_task in asyncio.as_completed(tasks):
            ...
            completed_count += 1
            elapsed = time.time() - overall_start
            print(f"   Call {result['call_id']} completed #{completed_count}/{num_calls} at {elapsed:.1f}s")
        ...
        if total_time > 15:
            print(f"   🚨 LIKELY SEQUENTIAL: {total_time:.1f}s suggests thread pool exhaustion")
        else:
            print(f"   ✅ GOOD CONCURRENCY: {total_time:.1f}s suggests parallel execution")
