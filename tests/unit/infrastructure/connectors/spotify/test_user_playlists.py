...
class TestNullableImagesCoercion:
    ...
    arrive as `null` on SimplifiedPlaylistObject. The response model must
    coerce this to `[]` rather than fail validation. Regression test for a
    user-reported import crash: `2 validation errors for SpotifyUserPlaylistsResponse`.
    """
    ...
    def test_mixed_null_and_populated_images_parses(self) -> None:
        ...
        with_art = _playlist_item("a", "A")
        with_art["images"] = [
            {"url": "https://i.scdn.co/image/abc", "width": 300, "height": 300}
        ]
        no_art = _playlist_item("b", "B")
        no_art["images"] = None
