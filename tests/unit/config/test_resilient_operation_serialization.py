...
class TestResilientOperationConcurrency:
    ...
    async def test_resilient_operation_allows_concurrency(self):
        ...
        assert total_duration < (operation_delay + 0.1), (
            f"Expected ~{operation_delay}s, got {total_duration:.3f}s - indicates serialization!"
        )
        ...
        assert start_spread < 0.1, (
            f"Operations should start concurrently, got {start_spread:.3f}s spread"
        )
    ...
    async def test_resilient_operation_with_asyncio_to_thread(self):
        ...
        assert total_duration < 0.6, (
            f"Expected concurrent execution (~0.3s), got {total_duration:.3f}s"
        )
    ...
    async def test_mock_lastfm_pattern_with_decorator(self):
        ...
        if total_duration > 0.5:
            print(
                "   🚨 SERIALIZATION DETECTED: resilient_operation may be the culprit!"
            )
        else:
            print("   ✅ CONCURRENT EXECUTION: resilient_operation is NOT the issue")

        assert total_duration < 0.5, (
            f"Expected concurrent execution, got {total_duration:.3f}s"
        )
        assert unique_threads > 1, f"Expected multiple threads, got {unique_threads}"
