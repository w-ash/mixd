...
class TestLastFMAPICallAnalysis:
    ...
    async def test_metadata_extraction_api_call_count(self):
        ...
        def create_api_call_mock(method_name):
            ...
            def mock_method(*args, **kwargs):
                ...
                print(
                    f"🔄 API Call #{api_call_count}: {method_name} took {duration * 1000:.1f}ms"
                )
        ...
        print(
            f"📊 Expected API calls based on EXTRACTORS: {len(LastFMTrackInfo.EXTRACTORS)}"
        )
        ...
        print(
            f"📊 Average per API call: {(total_duration / api_call_count) * 1000:.1f}ms"
            if api_call_count > 0
            else "No calls"
        )
        print(
            f"🎯 Expected with real API delays (~100ms each): {api_call_count * 100:.0f}ms"
        )
        ...
        print(
            "   - Each pylast method (get_title, get_mbid, etc.) is a separate API call"
        )
        print(
            f"   - With real API latency (~100ms), this would take ~{api_call_count * 100}ms"
        )
    ...
    async def test_proposed_single_api_call_approach(self):
        ...
        result_data = {
            ...
            "lastfm_user_playcount": int(
                single_api_call_data["track"]["userplaycount"]
            ),
        ...
        print(f"⏱️  Total time taken: {total_duration * 1000:.1f}ms")
        print(
            f"📊 All metadata fields extracted: {len([k for k, v in result_data.items() if v is not None])}"
        )
        ...
        print(
            f"   Proposed approach: 1 API call × 100ms = ~{total_duration * 1000:.0f}ms"
        )
    ...
    def test_lastfm_extractors_count(self):
        ...
        assert extractor_count >= 10, (
            f"Expected at least 10 extractors, found {extractor_count}"
        )

        print("\n💡 This confirms why metadata conversion takes 1,400-1,700ms:")
        print(
            f"   {extractor_count} API calls × ~100-120ms per call = ~{extractor_count * 110}ms"
        )
