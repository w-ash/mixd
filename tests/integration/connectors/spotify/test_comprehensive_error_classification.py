...
class TestComprehensiveErrorClassification:
    ...
    async def test_permanent_http_errors_no_retry_comprehensive(
        ...
        with patch(
            "src.infrastructure.connectors.spotify.client.spotify_api_call"
        ) as mock_api_call:
            ...
            assert call_count == 1, (
                f"Expected 1 call for permanent error {status_code}, got {call_count}"
            )
    ...
    async def test_not_found_error_no_retry(self, spotify_client):
        ...
        def mock_get_tracks_not_found(*args, **kwargs):
            ...
            exception = spotipy.SpotifyException(
                404, -1, "Not Found - resource doesn't exist"
            )
        ...
        with patch(
            ...
            assert call_count == 1, (
                f"Expected 1 call for not found error, got {call_count}"
            )
    ...
    async def test_rate_limit_error_retries(self, spotify_client):
        ...
        def mock_get_tracks_rate_limit(*args, **kwargs):
            ...
            call_count += 1
            exception = spotipy.SpotifyException(
                429, -1, "Too Many Requests - rate limit exceeded"
            )
        ...
        with patch(
            ...
            assert call_count == 3, (
                f"Expected 3 calls for rate limit error, got {call_count}"
            )

            # Should have some retry delays to prevent overwhelming servers (not testing precise timing)
            assert duration > 0, (
                f"Rate limit retries should have some delay: {duration}s"
            )
    ...
    async def test_temporary_server_errors_retry_comprehensive(
        ...
        with patch(
            ...
            # Should retry 3 times total (max_tries=3)
            assert call_count == 3, (
                f"Expected 3 calls for server error {status_code}, got {call_count}"
            )

            # Should have retry delays to prevent overwhelming servers (not testing precise timing)
            assert duration > 0, (
                f"Server error retries should have some delay: {duration}s"
            )

    # TEXT PATTERN ERRORS - Should classify based on message content
    @pytest.mark.parametrize(
        ("error_message", "expected_type", "expected_retries"),
        [
    ...
    @pytest.mark.asyncio
    async def test_text_pattern_error_classification(
        self, spotify_client, error_message, expected_type, expected_retries
    ):
        ...
        with patch(
            ...
            assert call_count == expected_retries, (
                f"Expected {expected_retries} calls for {expected_type} error '{error_message}', got {call_count}"
            )
            ...
            if expected_type in ["permanent", "not_found"]:
                # Should be fast (no retry delays)
                assert duration < 2.0, (
                    f"{expected_type} error took too long: {duration}s"
                )
            else:
                # Should have retry delays for retriable errors
                assert duration > 0, (
                    f"{expected_type} error retries should have some delay: {duration}s"
                )
    ...
    async def test_network_errors_propagate_up(self, spotify_client, network_error):
        ...
        with patch(
            ...
            assert call_count == 1, (
                f"Expected 1 call for network error {type(network_error).__name__}, got {call_count}"
            )
    ...
    async def test_success_after_temporary_failure(self, spotify_client):
        ...
        with patch(
            ...
            assert call_count == 3, (
                f"Expected 3 calls for eventual success, got {call_count}"
            )
    ...
    async def test_all_methods_error_handling_comprehensive(
        self, spotify_client, method_name, method_args
    ):
        ...
        with patch(
            ...
            if method_name == "playlist_change_details":
                ...
                assert (
                    result is None
                )  # Will actually raise, but let's check the pattern
            else:
                assert result is None

            # Should retry 3 times for rate limit errors
            assert call_count == 3, (
                f"Method {method_name} expected 3 calls for rate limit error, got {call_count}"
            )
