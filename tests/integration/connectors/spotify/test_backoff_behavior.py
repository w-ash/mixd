...
class TestSpotifyBackoffBehavior:
    ...
    async def test_exponential_backoff_timing_temporary_errors(self, spotify_client):
        ...
        with patch(
            ...
            assert total_duration > 0.1, (
                f"Retries should have delays for server protection: {total_duration}s"
            )
            ...
            if len(call_times) >= 2:
                ...
                assert first_retry_delay > 0, (
                    f"First retry should have delay: {first_retry_delay}s"
                )
    ...
    async def test_immediate_failure_permanent_errors(self, spotify_client):
        ...
        with patch(
            ...
            assert total_duration < 1.0, (
                f"Permanent error took too long: {total_duration}s"
            )
    ...
    async def test_error_logging_and_classification_behavior(self, spotify_client):
        ...
        with (
            ...
            patch(
                "src.infrastructure.connectors._shared.error_classification.logger"
            ) as mock_logger,
        ):
            ...
            rate_limit_logs = [
                log for log in captured_logs if "rate limit" in log.lower()
            ]
            assert len(rate_limit_logs) > 0, (
                "Rate limit detection must be logged for operators"
            )
    ...
    async def test_giveup_condition_integration(self, spotify_client):
        ...
        with patch(
            ...
            assert call_count == 2, (
                f"Expected 2 calls due to giveup condition, got {call_count}"
            )
    ...
    async def test_successful_retry_after_failures(self, spotify_client):
        ...
        with patch(
            ...
            assert total_duration >= 0.2, (
                f"Successful retry too fast: {total_duration}s"
            )
    ...
    @pytest.mark.asyncio
    async def test_all_methods_have_backoff_decorators(
        self, spotify_client, method_info
    ):
        ...
        with patch(
            ...
            assert result is None
            assert call_count == 3, (
                f"Method {method_name} should retry 3 times, got {call_count}"
            )
            # Must have some retry delays to avoid overwhelming servers
            assert total_duration > 0, (
                f"Method {method_name} should have retry delays: {total_duration}s"
            )
    ...
    async def test_wrapper_pattern_exception_handling(self, spotify_client):
        ...
        with patch(
            ...
            assert (
                call_count == 3
            )  # Should have exhausted all retries before re-raising
