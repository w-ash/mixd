...
class TestComprehensiveErrorClassification:
    ...
    @pytest.mark.asyncio
    async def test_permanent_error_no_retry_comprehensive(self, lastfm_client, error_code, description):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            assert call_count == 1, f"Expected 1 call for permanent error {error_code}, got {call_count}"
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Performance: This test takes too long due to real backoff delays - covered by faster tests")
    async def test_temporary_error_retry_comprehensive(self, lastfm_client, error_code, description):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            assert result is not None or call_count == 2  # Either succeeds with data or retried correctly
            
            # Should have retried (2 calls total: 1 failure + 1 success)
            assert call_count == 2, f"Expected 2 calls for temporary error {error_code}, got {call_count}"
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Performance: This test takes too long due to real backoff delays - covered by faster tests")
    async def test_rate_limit_retry_comprehensive(self, lastfm_client, rate_limit_variant):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            assert result is not None or call_count == 3  # Either succeeds or retried correctly
            
            # Should have retried (3 calls total: 2 failures + 1 success)
            assert call_count == 3, f"Expected 3 calls for rate limit variant {rate_limit_variant}, got {call_count}"
    ...
    @pytest.mark.parametrize(('error_pattern', 'expected_type', 'should_retry'), [
    ...
    @pytest.mark.asyncio
    async def test_text_pattern_classification(self, lastfm_client, error_pattern, expected_type, should_retry):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            if should_retry:
                ...
                assert call_count == 2, f"Expected retry for {expected_type} error: {error_pattern}"
            else:
                # Should fail immediately for non-retryable text patterns
                assert result is None
                assert call_count == 1, f"Expected no retry for {expected_type} error: {error_pattern}"
    ...
    async def test_unknown_error_handling(self, lastfm_client):
        ...
        def mock_get_track_unknown_error(*args, **kwargs):
            ...
            if call_count <= 1:
                ...
                raise pylast.WSError("LastFm", "9999", "Completely unknown error that should be retried")
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            assert call_count == 2, f"Expected retry for unknown error, got {call_count} calls"
    ...
    async def test_non_pylast_exception_handling(self, lastfm_client):
        ...
        with patch('pylast.LastFMNetwork') as mock_network_class:
            ...
            assert call_count == 1, f"Expected 1 call for programming error, got {call_count}"
    ...
    async def test_error_classifier_integration(self, lastfm_client):
        ...
        for exception, expected_type, should_retry in test_cases:
            error_type, _error_code, _error_description = classifier.classify_error(exception)
            
            # Verify classification matches expectations
            assert error_type == expected_type, f"Expected {expected_type}, got {error_type} for {exception}"
            ...
            assert should_giveup != should_retry, f"Giveup logic mismatch for {error_type}: should_giveup={should_giveup}, should_retry={should_retry}"
...
class TestErrorClassificationEdgeCases:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Performance: This test takes 60+ seconds due to real exponential backoff delays")
