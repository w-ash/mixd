...
class TestLastFMErrorHandlingPerformance:
    ...
    def mock_settings(self):
        ...
        with patch("src.config.settings") as mock_settings:
            ...
            mock_settings.api.lastfm_rate_limit = (
                100.0  # High rate for performance testing
            )
    ...
    async def test_successful_calls_performance_baseline(self, lastfm_client):
        ...
        with (
            patch("pylast.LastFMNetwork") as mock_network_class,
            patch.object(
                LastFMAPIClient, "_get_comprehensive_track_data", return_value=mock_data
            ),
        ):
            ...
            for i in range(num_calls):
                task = lastfm_client.get_track_info_comprehensive(
            ...
            print(
                f"✅ Successful calls: {num_calls} calls in {duration:.2f}s ({calls_per_second:.1f} calls/sec)"
            )
    ...
    async def test_mixed_scenario_performance(self, lastfm_client):
        ...
        with (
            ...
            patch.object(
                LastFMAPIClient,
                "_get_comprehensive_track_data",
                return_value=mock_success_data,
            ),
        ):
            mock_network = MagicMock()
            ...
            print(
                f"✅ Mixed scenario: {len(results)} calls in {duration:.2f}s ({success_rate:.1f}% success rate)"
            )
    ...
    async def test_error_classification_overhead(self, lastfm_client):
        ...
        with patch("pylast.LastFMNetwork") as mock_network_class:
            ...
            print(
                f"✅ Error classification overhead: {overhead_per_call:.3f}s per call for {num_calls} calls"
            )
