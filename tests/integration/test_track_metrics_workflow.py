...
class TestTrackMetricsWorkflowIntegration:
    ...
    async def test_spotify_popularity_workflow_prevents_inf_values_regression(self, db_session):
        ...
        assert "spotify_popularity" in metrics, "Spotify popularity metrics should be present"
        ...
        for track_id, popularity in popularity_values.items():
            assert popularity > 0, f"Track {track_id} has invalid popularity: {popularity}"
            assert popularity != float('-inf'), f"Track {track_id} has -inf popularity (REGRESSION)!"
            assert isinstance(popularity, (int, float)), f"Track {track_id} popularity not numeric: {type(popularity)}"
        
        # Verify specific expected values retrieved from database
        assert popularity_values[track_ids[0]] == 85, "Track 1 should have database value 85"
        assert popularity_values[track_ids[1]] == 67, "Track 2 should have database value 67"
    ...
    async def test_metrics_enrichment_workflow_robustness(self, db_session):
        ...
        assert track_id in metrics["spotify_popularity"], "Should have metrics for the test track"
        
        # Verify no invalid values (the key regression prevention)
        popularity_value = metrics["spotify_popularity"][track_id]
        assert isinstance(popularity_value, (int, float)), "Popularity should be numeric"
        assert popularity_value != float('-inf'), "Should never return -inf values (CRITICAL)"
