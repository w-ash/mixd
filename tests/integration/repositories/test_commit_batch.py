...
def _make_test_track(suffix: str) -> Track:
    ...
    return Track(
        ...
        artists=[Artist(name=f"TEST_Artist_{suffix}_{uid}")],
    )
...
class TestCommitBatch:
    ...
    async def test_both_batches_persist(self, db_session, test_data_tracker):
        ...
        async with uow:
            ...
            test_data_tracker.add_track(saved_a.id)
            await uow.commit_batch()

            saved_b = await repo.save_track(_make_test_track("B"))
            test_data_tracker.add_track(saved_b.id)
            await uow.commit_batch()
    ...
    async def test_rollback_does_not_undo_committed_batch(
    ...
    async def test_committed_flag_stays_false(self, db_session, test_data_tracker):
    ...
    @pytest.mark.slow
    async def test_multiple_batches_with_final_commit(
        self, db_session, test_data_tracker
    ):
