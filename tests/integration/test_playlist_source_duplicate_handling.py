...
class TestPlaylistSourceDuplicateHandling:
    ...
    @pytest.mark.asyncio
    async def test_connector_playlist_with_duplicates_preserves_all_instances(self, db_session):
        ...
        connector_playlist = ConnectorPlaylist(
            ...
            connector_playlist_id="test_playlist_with_duplicates",
            name="Test Playlist with Duplicates",
            items=[
                ConnectorPlaylistItem(
                    connector_track_id="spotify_track_123",
                    position=0,
                ...
                ConnectorPlaylistItem(
                    connector_track_id="spotify_track_456", 
                    position=1,
                ...
                ConnectorPlaylistItem(
                    connector_track_id="spotify_track_123",  # Duplicate of position 0
                    position=2,
        ...
        with patch.object(uow, 'get_service_connector_provider') as mock_provider:
            ...
            async with uow:
                ...
                command = CreateCanonicalPlaylistCommand(
                    ...
                    metadata={"test_id": "duplicate_handling_test", "source": "integration_test"}
                )
                ...
                assert tracks[0].connector_track_ids["spotify"] == tracks[2].connector_track_ids["spotify"]
                assert tracks[0].connector_track_ids["spotify"] != tracks[1].connector_track_ids["spotify"]
                
                # Verify: Core business requirement - all track instances preserved including duplicates
                assert len(result.playlist.tracks) == 3, "All track instances should be preserved"
                assert result.playlist.name == "Test Integration Playlist"
                ...
                assert result.playlist.id is not None, "Playlist should be persisted with ID"

    @pytest.mark.asyncio
    async def test_empty_connector_playlist_processing_returns_empty_tracklist(self, db_session):
        ...
        empty_connector_playlist = ConnectorPlaylist(
            connector_name="spotify",
            connector_playlist_id="empty_test_playlist",
