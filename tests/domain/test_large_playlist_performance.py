...
class TestLargePlaylistPerformance:
    ...
    def large_playlist_5k(self):
        ...
        tracks = [
            Track(id=i, title=f"Track {i:04d}", artists=[Artist(name=f"Artist {i:04d}")])
            for i in range(5000)
        ]
    ...
    def large_playlist_10k(self):
        ...
        tracks = [
            Track(id=i, title=f"Track {i:05d}", artists=[Artist(name=f"Artist {i:05d}")])
            for i in range(10000)
        ]
        return Playlist(name="10K Test Playlist", tracks=tracks)

    def test_5k_playlist_idempotency_performance(self, large_playlist_5k):
    ...
    def test_10k_playlist_idempotency_performance(self, large_playlist_10k):
    ...
    def test_5k_playlist_complete_reversal(self, large_playlist_5k):
        ...
        # Should complete efficiently (allow up to 3s for worst-case 5K reversal)
        assert execution_time < 3.0
        print(f"5K playlist complete reversal: {execution_time:.3f}s, {len(move_ops)} moves")

    def test_5k_playlist_partial_reorder(self, large_playlist_5k):
        ...
        print(f"5K playlist partial reorder: {execution_time:.3f}s, {len(move_ops)} moves")

    def test_api_strategy_large_playlist(self, large_playlist_5k):
    ...
    def test_canonical_strategy_large_playlist(self, large_playlist_5k):
    ...
    def test_duplicate_heavy_playlist_performance(self):
    ...
    def test_memory_efficiency_large_playlist(self, large_playlist_10k):
        ...
        target_tracks = large_playlist_10k.tracks[1000:] + large_playlist_10k.tracks[:1000]
        target_tracklist = TrackList(tracks=target_tracks)
    ...
    def test_concurrent_large_playlist_operations(self, large_playlist_5k):
        ...
        async def test_concurrent():
            ...
            scenarios = [
                ...
                large_playlist_5k.tracks[100:] + large_playlist_5k.tracks[:100],  # Rotation
                large_playlist_5k.tracks[::2] + large_playlist_5k.tracks[1::2],  # Interleave
            ]
    ...
    def test_stress_test_10k_worst_case(self, large_playlist_10k):
