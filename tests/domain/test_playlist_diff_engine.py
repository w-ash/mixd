...
class TestLongestIncreasingSubsequence:
    ...
    def test_duplicates_in_sequence(self):
        ...
        # Verify LIS is valid
        lis_values = [sequence[i] for i in result]
        assert all(lis_values[i] < lis_values[i + 1] for i in range(len(lis_values) - 1))
...
class TestLISReorderOperations:
    ...
    def test_single_track_move(self, sample_tracks):
        ...
        current = sample_tracks.copy()  # [1, 2, 3, 4, 5]
        target = [sample_tracks[1], sample_tracks[0], sample_tracks[2], sample_tracks[3], sample_tracks[4]]  # [2, 1, 3, 4, 5]
    ...
    def test_partial_reorder_with_lis_optimization(self, sample_tracks):
        ...
        target = [sample_tracks[0], sample_tracks[2], sample_tracks[3], sample_tracks[4], sample_tracks[1]]
...
class TestPlaylistDiffIntegration:
    ...
    def test_complex_mixed_operations(self, sample_playlist):
        ...
        remaining_tracks = [t for t in sample_playlist.tracks if t.id != 2]  # Remove track 2
        new_track = Track(id=5, title="Track E", artists=[Artist(name="Artist 5")])
        # Reorder: [new_track, track_4, track_1, track_3]
        target_tracks = [new_track, remaining_tracks[2], remaining_tracks[0], remaining_tracks[1]]
        target_tracklist = TrackList(tracks=target_tracks)
