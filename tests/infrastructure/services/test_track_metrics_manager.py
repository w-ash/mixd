...
class TestTrackMetricsManagerCriticalFlow:
    ...
    def sample_tracks_with_ids(self):
        ...
        return TrackList(
            tracks=[
                ...
                Track(
                    id=2, title="Less Popular Song", artists=[Artist(name="Artist 2")]
                ),
            ]
        )
    ...
    async def test_existing_metrics_prevent_api_calls_regression_prevention(
        ...
        metrics_repo.get_track_metrics.return_value = existing_metrics[
            "spotify_popularity"
        ]
    ...
    async def test_missing_metrics_trigger_fresh_fetch_only_for_missing(
        ...
        # Act
        _enriched_tracklist, metrics = await metrics_manager.enrich_tracks(
            sample_tracks_with_ids, "spotify", Mock(), spotify_popularity_extractors
        )
...
class TestGetExistingMetrics:
    ...
    async def test_get_existing_metrics_returns_database_values(
        self, metrics_manager_simple
    ):
        ...
        metrics_manager_simple.metrics_repo.get_track_metrics.side_effect = (
            mock_get_track_metrics
        )
    ...
    async def test_get_existing_metrics_handles_empty_inputs(
        self, metrics_manager_simple
    ):
    ...
    async def test_get_existing_metrics_handles_database_errors(
        self, metrics_manager_simple
    ):
        """Test error handling when database queries fail."""
        # Arrange
        metrics_manager_simple.metrics_repo.get_track_metrics.side_effect = Exception(
            "DB Error"
        )
...
class TestMetricsPersistence:
    ...
    async def test_persist_metrics_saves_fresh_values(self, metrics_manager_simple):
        ...
        # Act
        await metrics_manager_simple._persist_metrics_to_database(
            fresh_metrics, "spotify"
        )

        # Assert
        metrics_manager_simple.metrics_repo.save_track_metrics.assert_called_once()
        saved_batch = metrics_manager_simple.metrics_repo.save_track_metrics.call_args[
            0
        ][0]
